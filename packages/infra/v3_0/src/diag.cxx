...
#define TELNET_BUF_LENGTH	512
static char telnetBuf[TELNET_BUF_LENGTH + 10] = {0};
static int PRINTF_FLAG = 0;
externC void telnet_printf(char * message, int messagelen); 
...
_vprintf(void (*putc)(char c, void **param), void **param, const char *fmt, va_list ap)
{
    ...
    while ((c = *fmt++) != '\0') {
        if ((PRINTF_FLAG) 
          && (pTelnetBuf - &telnetBuf[0]) >= TELNET_BUF_LENGTH) {
            telnet_printf(&telnetBuf[0], (pTelnetBuf - &telnetBuf[0]));
            pTelnetBuf = &telnetBuf[0];
        }
        if (c == '%') {
            c = *fmt++;
            ...
            if (!pad_on_right) {
                ...
                if (PRINTF_FLAG) {
                        *(pTelnetBuf++) = c;
                        if ((pTelnetBuf - &telnetBuf[0]) >= TELNET_BUF_LENGTH) {
                          telnet_printf(&telnetBuf[0], (pTelnetBuf - &telnetBuf[0]));
                          pTelnetBuf = &telnetBuf[0];
                        }
                }					
