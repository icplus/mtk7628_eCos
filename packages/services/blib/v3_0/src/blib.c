...
blib_sync_block(cyg_blib_t *bl, blib_block_t *block)
{
    ...
    if (block->modified)
    {
        cyg_uint32 len = bl->block_size / sector_size;

        D(("blib writting block=%d,block_data=%p\n", block->num,block->data)); 
        STAT(bl, n_writes);
               ...
               blib_block_t **dblock)
{
    ...
    if (read_data)
    {
        D(("blib reading block=%d block_size=%d sector_size=%d\n", block->num,bl->block_size,sector_size)); 

        STAT(bl, n_reads);

        len = bl->block_size / sector_size;
        //D(("len=%d block_size=%d sector_size=%d\n", len,bl->block_size,sector_size)); 
...
{
    cyg_uint64 posl = (cyg_uint64)pos;
    return cyg_io_bread((cyg_io_handle_t)priv, buf, len, posl);
}
...
{
    cyg_uint64 posl = (cyg_uint64) pos;
    return cyg_io_bwrite((cyg_io_handle_t)priv, buf, len, posl);
}
...
int
cyg_blib_write_block(cyg_blib_t *bl,
               const void *buf,
               cyg_uint32 *len,
               cyg_uint32  bnum,
               cyg_uint32  pos)
{
    ...
    D(("blib write len=%d pos=%d bnum=%d block_size=%d\n", *len, pos, bnum,bl->block_size));
    while (size > 0)
    {
        cyg_uint32 csize;
