...
#define HAL_DCACHE_LINE_ADDRESS(_addr_) \
(((CYG_ADDRESS)(_addr_)) & ~(HAL_DCACHE_LINE_SIZE-1))

#define HAL_ICACHE_LINE_ADDRESS(_addr_) \
...
#define HAL_DCACHE_INDEX_WB_INVALIDATE      0x0
...
#define HAL_DCACHE_HIT_WB_INVALIDATE        0x5
...
#define HAL_DCACHE_SYNC() 							\
    ...
    {                                                                                   \
        asm volatile (" cache %0, 0(%1)"                                                \
                      :                                                                 \
                      : "I" (HAL_CACHE_OP(HAL_WHICH_DCACHE, HAL_DCACHE_INDEX_WB_INVALIDATE)),      \
                        "r"(addr));                                                     \
    }                                                                                   \
    CYG_MACRO_END
...
#define HAL_DCACHE_LOCK(_base_, _asize_)                                                \
    CYG_MACRO_START                                                                     \
    register CYG_ADDRESS _eaddr_ = ((CYG_ADDRESS)(_base_) + (_asize_));                 \
    register CYG_ADDRESS _addr_ = HAL_DCACHE_LINE_ADDRESS(_base_);                      \
...
#define HAL_DCACHE_UNLOCK(_base_, _asize_)                                              \
    ...
    for( ; _addr_ < _eaddr_; _addr_ += HAL_DCACHE_LINE_SIZE )                           \
      asm volatile (" cache %0, 0(%1)"                                                  \
                    :                                                                   \
                    : "I" (HAL_CACHE_OP(HAL_WHICH_DCACHE, HAL_DCACHE_HIT_WB_INVALIDATE)),         \
                      "r"(_addr_));                                                     \
    CYG_MACRO_END
...
#else
//sht:comment:HAL_DCACHE_FLUSH may be not finish when DMA start to use this buf.check later.
#define HAL_DCACHE_FLUSH( _base_, _asize_)                                              \
    CYG_MACRO_START                                                                     \
    register CYG_ADDRESS _eaddr_ = ((CYG_ADDRESS)(_base_) + (_asize_));                 \
    register CYG_ADDRESS _addr_ = HAL_DCACHE_LINE_ADDRESS(_base_);                      \
...
#define HAL_ICACHE_LOCK(_base_, _asize_)                                                \
    CYG_MACRO_START                                                                     \
    register CYG_ADDRESS _eaddr_ = ((CYG_ADDRESS)(_base_) + (_asize_));                 \
    register CYG_ADDRESS _addr_ = HAL_ICACHE_LINE_ADDRESS(_base_);                      \
