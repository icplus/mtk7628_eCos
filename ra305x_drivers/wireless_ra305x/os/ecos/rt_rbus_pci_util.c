...
{
	PECOS_PKT_BUFFER        pTempPacket;
	struct mbuf     *pMBuf;
	int headroom, tailroom;

	pTempPacket = RTPKT_TO_OSPKT(pPacket);
	pMBuf = pTempPacket->pDataMBuf;	
	pMBuf->m_len = pTempPacket->pktLen;

	headroom = pTempPacket->pDataPtr - CYGARC_UNCACHED_ADDRESS(mtod(pMBuf, PUCHAR));  
	tailroom = M_TRAILINGSPACE(pMBuf);

	if  (headroom < ext_head_len)
		DBGPRINT(RT_DEBUG_ERROR, ("%s, can't expand headroom, headroom=%d, ext_head_len=%d\n", __FUNCTION__,headroom, ext_head_len));

	if (tailroom < ext_tail_len)
		DBGPRINT(RT_DEBUG_ERROR, ("%s, can't expand tailroom, tailroom=%d, ext_tail_len=%d\n", __FUNCTION__,tailroom, ext_tail_len));

	return OSPKT_TO_RTPKT(pTempPacket);	
}
...
extern void CFG_reset_default();
extern int mon_snd_cmd(int cmd);
