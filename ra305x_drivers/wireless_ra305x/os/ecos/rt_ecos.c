...
int printUI(const char *fmt, ...)
{
    ...
    DbgBuffer[ret] = '\0';
    if (ret > 0)
        SysLog(LOG_USER|SYS_LOG_INFO|LOGM_WLAN, DbgBuffer);	
    return (ret);
}
#endif /* BRANCH_ADV */
...
void dump_RxDesc(PRXD_STRUC pRxDesc)
{
	...
	printf("\tPlcpSignal=0x%x!\n", pRxDesc->PlcpSignal);
	printf("\tPlcpRssil=0x%x!\n", pRxDesc->PlcpRssil);
}

	
void dump_RxWI(PRXWI_STRUC pRxWI)
{
...
extern int flsh_erase(unsigned int addr, unsigned int len);
extern int flsh_write(unsigned int addr, unsigned int from, unsigned int len);
...
void RtmpFlashWrite(UCHAR *p, ULONG offset, ULONG size)
{
	...
	if (status == 0)
	{
		...
		if (status != 0)
			DBGPRINT(RT_DEBUG_ERROR, ("RtmpFlashWrite program fail\n"));
	} else
		DBGPRINT(RT_DEBUG_ERROR, ("RtmpFlashWrite erase fail\n"));
}
...
void RtmpFlashRead(UCHAR *p, ULONG a, ULONG b)
{
	...
	status = flsh_read(flsbase, rambase, len);
	if (status != 0)
		DBGPRINT(RT_DEBUG_ERROR, ("RtmpFlashRead fail\n"));
	...
	IN RTMP_OS_TASK *pTask)
{
	...
	RTMP_SEM_EVENT_UP(&pTask->taskSema);
        ...
        while (1) {
		cyg_semaphore_wait(&pTask->taskComplete.wait);
		...
		{
			DBGPRINT(RT_DEBUG_ERROR, ("Killed task(%s) donebit=%d\n",
				pTask->taskName, pTask->taskComplete.done)); 
		}
	}

        pTask->task_killed = 0;
        cyg_semaphore_destroy(&pTask->taskComplete.wait);
        cyg_semaphore_destroy(&pTask->taskSema);
