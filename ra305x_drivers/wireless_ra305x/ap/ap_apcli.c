	...
	OUT UCHAR		*pWcid)
{
	...
	if (ValidApCliEntry(pAd, apCliIdx))
	{
		...
		if (pAd->ApCfg.bMACRepeaterEn == TRUE)
		{
			...
			{
				if ((!MAC_ADDR_EQUAL(pAd->ApCfg.ApCliTab[apCliIdx].CurrentAddress, (pSrcBufVA + ETH_LENGTH_OF_ADDRESS))) &&
					(!MAC_ADDR_EQUAL(pAd->ApCfg.BridgeAddress, (pSrcBufVA + ETH_LENGTH_OF_ADDRESS))) &&
