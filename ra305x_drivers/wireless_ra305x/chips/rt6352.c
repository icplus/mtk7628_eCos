	...
	IN	PRTMP_ADAPTER pAd)
{
	...
	if (pAd->CommonCfg.Chip_VerID > 1)
	{
		DBGPRINT(RT_DEBUG_WARN, ("Initialize RF Central Registers for E2 !!!\n"));
	...
	{
		DBGPRINT(RT_DEBUG_WARN, ("Initialize RF Central Registers for E3 !!!\n"));
	...
	if (pAd->CommonCfg.Chip_VerID > 1)
	{
		DBGPRINT(RT_DEBUG_WARN, ("Initialize RF Channel Registers for E2 !!!\n"));
	...
	if ((pAd->CommonCfg.Chip_VerID > 1) && (pAd->CommonCfg.Chip_E_Number >= 2))
	{
		DBGPRINT(RT_DEBUG_WARN, ("Initialize RF Channel Registers for E3 !!!\n"));
	...
	{
		DBGPRINT(RT_DEBUG_WARN, ("Initialize RF Channel Registers for DRQFN !!!\n"));
	...
	if (pAd->CommonCfg.Chip_VerID > 1)
	{
		DBGPRINT(RT_DEBUG_WARN, ("Initialize RF DCCal Registers for E2 !!!\n"));
	...
	if ((pAd->CommonCfg.Chip_VerID > 1) && (pAd->CommonCfg.Chip_E_Number >= 2))
	{
		DBGPRINT(RT_DEBUG_WARN, ("Initialize RF DCCal Registers for E3 !!!\n"));
	...
	*/
		...
		if (Channel == FreqItems6352[index].Channel)
		{
			...
			if (bScan == FALSE)
			{
				...
				if ((pAd->CommonCfg.Chip_VerID > 1) && (pAd->CommonCfg.Chip_E_Number >= 2))
				{
					...
					/* ADC clcok selection */
					DBGPRINT(RT_DEBUG_ERROR, ("ADC clcok selection for E3 !!!\n"));
					{
						if (Channel > 10)
						{
	...
	if (bScan == FALSE)
	{
...
#endif /* RT6352_EP_SUPPORT */
		{
			...
			pAd->DoDPDCurrTemperature = (pAd->CurrTemperature - pAd->TemperatureRef25C) * 19;
		}
			...
			if(ant_idx == 0){
				switch (index){
					case 0:
						...
						DBGPRINT(RT_DEBUG_INFO, ("rt635x_tssi0_dc is 0x%08x\n", rt635x_tssi0_dc));
						break;
					case 1: 								
					...
					case 3:								
						...
						rt635x_tssi0_db_hvga = lin2dBd(tssi0_linear - rt635x_tssi0_dc_hvga) - tssi0_db;
						DBGPRINT(RT_DEBUG_INFO, ("tssi0_db_hvga is %d\n", rt635x_tssi0_db_hvga));
						break;
				}
			}
			else{
				switch (index){
					...
					case 3:				
						...
						rt635x_tssi1_db_hvga = lin2dBd(tssi1_linear - rt635x_tssi1_dc_hvga) - tssi1_db;
	...
	IN	CHAR		percent_delta)
{
	...
	{
		DBGPRINT(RT_DEBUG_INFO, ("chain %d: base_power is %d\n", chain, base_power[chain]));
		...
		DBGPRINT(RT_DEBUG_INFO, ("tssi_use_hvga %d \n", tssi_use_hvga[chain]));
	}
...
//	    if ((base_power[chain] + cur_comp_power) >= 0)
		...
		DBGPRINT(RT_DEBUG_INFO, ("pwr is %d \n", pwr));
...
VOID RT635xTriggerTssiCompensation(
	IN	PRTMP_ADAPTER	pAd,
	IN	CHAR		percent_delta)
{
	UINT32 MacValue;
	INT32 base_power[max_ant];
	INT32 target_power = 0;
	UCHAR RFValue, BBPValue, tssi_use_hvga[max_ant];
	UCHAR chain;
	UCHAR max_stream = 1;
	...
	pAd->TxPowerCtrl.bTriggerPwrCompensation = TRUE;
			...
			for (j = 0; j < 8; j += 2)
			{
				...
				DBGPRINT(RT_DEBUG_TRACE, 
					("[%d, %d] channel_power(%d) + mcs_pwr_table(%d) + pa_mode(%d, %d) = rate_pwr(%d)\n",
						i, j, channel_power, rate_pwr_ori, pa_mode, pa_mode_table[pa_mode], rate_pwr));

				DBGPRINT(RT_DEBUG_TRACE, 
					("[%d, %d] sku_rate_pwr(%d) - rate_pwr(%d) = diff(%d). mcs_pwr_table_value(%d)\n",
						i, j, sku_rate_pwr, rate_pwr, diff, mcs_pwr_value));
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (pAd->Mlme.OneSecPeriodicRound % 4 == 0)
	{
		...
		channel_power = RT6352_AdjustChannelPwr(pAd, TotalDeltaPower);
		...
		if (pAd->TxPowerCtrl.bInternalTxALC == TRUE)
		{
			...
			{
				RT635xTssiCompensation(pAd, TotalDeltaPower);
#ifdef RTMP_TEMPERATURE_CALIBRATION
...
#ifdef RTMP_TEMPERATURE_CALIBRATION
		{
			if (pAd->bCalibrationDone)
			{
				...
				DBGPRINT(RT_DEBUG_WARN, ("Current Temperature from BBP_R49=0x%x\n", pAd->CurrTemperature));
				RT6352_TemperatureCalibration(pAd);
			}
		}
#endif /* RTMP_TEMPERATURE_CALIBRATION */
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (pAd->bRef25CVaild == FALSE)
	{
		...
		pAd->TemperatureRef25C = BBPR49 - 0x0F;
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if ((pAd->CommonCfg.PKG_ID == 1) && (pAd->bExtPA))
	{
		DBGPRINT_RAW(RT_DEBUG_ERROR, ("%s: Init Ext-PA BBP. \n", __FUNCTION__));
		RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R80, 0x4C);
		RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R82, 0xB3);
	...
	if ((pAd->CommonCfg.PKG_ID == 1) && (pAd->bExtPA))
	{
		...
		//RTMP_IO_WRITE32(pAd, TX_ALG_CFG_1, 0x4954002C); /* WH, 2012-12-28 */
