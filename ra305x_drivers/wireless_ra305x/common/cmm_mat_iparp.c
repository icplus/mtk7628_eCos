	...
	IN PUCHAR 			pDevMacAdr)
{
...
#ifdef MAC_REPEATER_SUPPORT
	{
		...
		if (pAd)
		{
			...
			pReptEntry = \
				&pAd->ApCfg.ApCliTab[pEntry->MatchAPCLITabIdx].RepeaterCli[pEntry->MatchReptCliIdx];
		}
		else
		{
		...
		if ((pEntry->bReptCli) && (pAd->ApCfg.bMACRepeaterEn) && (pAd->ApCfg.MACRepeaterOuiMode != 1))
		{
