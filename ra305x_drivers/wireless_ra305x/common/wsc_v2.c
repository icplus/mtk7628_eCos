	...
	IN  PUCHAR			pMacAddr)
{
	...
	if (bFound == FALSE)
	{
		...
		pACL->Num++;
#ifdef __ECOS
		/* Write ACLList to Flash for WPA2 TestPlan */
#define AccessControlList0 0x4A60200
#define AccessControlList1 0x4A70200
#define AccessControlList2 0x4A80200
#define AccessControlList3 0x4A90200
				UCHAR ACList[MAC_ADDR_LENGTH*10] = {0};
				UCHAR Temp[MAC_ADDR_LENGTH*10] = {0};		
					for (i=0; i<pACL->Num; i++) {
					if (i == 0) {
								sprintf(ACList, "%02x:%02x:%02x:%02x:%02x:%02x"
								, pACL->Entry[i].Addr[0], pACL->Entry[i].Addr[1], pACL->Entry[i].Addr[2]
								, pACL->Entry[i].Addr[3], pACL->Entry[i].Addr[4], pACL->Entry[i].Addr[5]);
					} else if (i < 10) {
								sprintf(Temp, "%s;%02x:%02x:%02x:%02x:%02x:%02x"
								, ACList
								, pACL->Entry[i].Addr[0], pACL->Entry[i].Addr[1], pACL->Entry[i].Addr[2]
								, pACL->Entry[i].Addr[3], pACL->Entry[i].Addr[4], pACL->Entry[i].Addr[5]);
					   sprintf(ACList, "%s", Temp);
					}
					}
		
						switch (ApIdx) {
								case 0:
										CFG_set_str(AccessControlList0,ACList); 			   
										break;
								case 1:
										CFG_set_str(AccessControlList1,ACList); 			   
										break;
								case 2:
										CFG_set_str(AccessControlList2,ACList); 			   
										break;
								case 3:
										CFG_set_str(AccessControlList3,ACList); 			   
										break;
						}
				  CFG_save(0);
#endif                
