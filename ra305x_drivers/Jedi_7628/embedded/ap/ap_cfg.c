	...
	IN RTMP_STRING *arg)
{
	...
	/* Save the auto connect flag, Lea */
	if (scan_mode == 0)
	{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("I/F(apcli%d) Set_ApCli_AutoConnect_Proc::(Len=%d,Ssid=%s), scan_mode=%ld\n",
			ifIndex, pApCfg->ApCliTab[ifIndex].CfgSsidLen, pApCfg->ApCliTab[ifIndex].CfgSsid, scan_mode));
