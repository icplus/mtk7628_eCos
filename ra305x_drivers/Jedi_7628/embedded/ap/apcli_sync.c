	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ApCli SYNC - ApCliMlmeProbeReqAction(Ssid %s, "
		"Bssid %02x:%02x:%02x:%02x:%02x:%02x)\n", Info->Ssid, PRINT_MAC(Info->Bssid)));
	...
	/* Initializing from configurations */
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ApCli SYNC - ifIndex = %u, CfgSSID = %s, CfgBSSID = %s\n", 
		ifIndex, 
		pAd->ApCfg.ApCliTab[ifIndex].CfgSsid, 
		pAd->ApCfg.ApCliTab[ifIndex].CfgApCliBssid));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ApCli SYNC - Start Probe the SSID %s(BSSID %s) on channel =%d\n", 
		pApCliEntry->MlmeAux.Ssid, pApCliEntry->MlmeAux.Bssid, pApCliEntry->MlmeAux.Channel));
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
								...
								TRUE))
	{
		...
		*/
		...
		LEA_PRINT("BSSID = %s, Channel = %u\n", 
			ie_list->Bssid, ie_list->Channel);
	...
	IN USHORT ifIndex)
{
	...
	else
	{
		...
		{
			/* Trying to really sendout the package */ 
			NDIS_STATUS status = MiniportMMRequest(pAd, QID_AC_BE, pOutBuffer, FrameLen);
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[ %s:%u ] MiniportMMRequest Sent out 0x%x, FrameLen = %d, returns %d(%s)\n", 
				__func__, __LINE__,
				pOutBuffer, FrameLen, 
				status, status == NDIS_STATUS_FAILURE ? "NDIS_STATUS_FAILURE" : (status == NDIS_STATUS_SUCCESS ? 
				"NDIS_STATUS_SUCCESS" : "Unkown")));
		}
