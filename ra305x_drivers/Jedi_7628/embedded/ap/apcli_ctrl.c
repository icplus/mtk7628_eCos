	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	PWSC_CTRL	pWpsCtrl = &pAd->ApCfg.ApCliTab[ifIndex].WscControl;
 	...
 	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[ %s:%u ] Start Probe Req, ifIndex = %u, CurrState = %u(%s)\
 	\n\tMAX_APCLI_NUM=%u, BSSID = %s(%s), Channel = %u, ApScanRunning? %s\n", 
				__FUNCTION__, __LINE__,
				ifIndex,
				pAd->Mlme.ApSyncMachine.CurrState, apcli_sm_str(APCLI_CTRL_STATE_MACHINE, pAd->Mlme.ApSyncMachine.CurrState), 
				MAX_APCLI_NUM, 
				pAd->ApCfg.ApCliTab[ifIndex].Ssid, pAd->ApCfg.ApCliTab[ifIndex].MlmeAux.Bssid, pAd->ApCfg.ApCliTab[ifIndex].MlmeAux.Channel,
				ApScanRunning(pAd) == TRUE ? "Yes" : "No"));
...
//		if (pAd->CommonCfg.Channel != pAd->ScanTab.BssEntry[bss_idx].Channel)
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[ %s:%u ] Probe Ssid=%s, Bssid=%02x:%02x:%02x:%02x:%02x:%02x\n",
		__FUNCTION__, __LINE__, JoinReq.Ssid, PRINT_MAC(JoinReq.Bssid)));
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[ %s:%u ] Probe Req Timeout, CurrState = %d(%s)\n", 
		__FUNCTION__, __LINE__, *pCurrState, apcli_sm_str(APCLI_CTRL_STATE_MACHINE, *pCurrState)));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[ %s:%u ] Probe Req Timeout. ProbeReqCnt=%d\n",
				__FUNCTION__, __LINE__, pApCliEntry->ProbeReqCnt));
	...
	*pCurrState = APCLI_CTRL_PROBE;
	...
	/* retry Probe Req. */
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[ %s:%u ] Retry Probe Req.\n", __FUNCTION__, __LINE__));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[ %s:%u ] Probe Ssid=%s, Bssid=%02x:%02x:%02x:%02x:%02x:%02x\n",
		__FUNCTION__, __LINE__, JoinReq.Ssid, JoinReq.Bssid[0], JoinReq.Bssid[1], JoinReq.Bssid[2],
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (Status == MLME_SUCCESS)
	{
		...
		LEA_PRINT("ProbeResp success. SSID = %s, Bssid = %02x:%02x:%02x:%02x:%02x:%02x\n", 
