...
static bool rti_ecos_init(struct cyg_netdevtab_entry *tab)
{
	...
	diag_printf("pAD=0x%x\n",pAd);
	pAd->infType = RTMP_DEV_INF_PCIE;//sht:for mt7603 porting temp usage.
	/*if (device_id == NIC7603_PCIe_DEVICE_ID) */
	{
		UINT32 val;

		RTMP_IO_READ32(pAd, 0x4000, &val);
		pAd->ChipID = val;
		if (IS_MT7603(pAd))
		{
			RTMP_IO_READ32(pAd, 0x1010, &val);
			pAd->AntMode = (val >> 24) & 0x1;
		}
...
static void  rti_ecos_stop(struct eth_drv_sc *sc)
{
	...
	if (pAd == NULL)
		return;
	MlmeRadioOn(pAd);//sht:sync with eCos AC750 patch to fix SW reboot hang
#ifdef APCLI_SUPPORT
	sprintf(devName, "%s0", SECOND_INF_APCLI_DEV_NAME);
	if (strcmp(pNetDev->dev_name, devName) == 0)
	{
...
bool rti_ecos_main_dev_init(char* devName)
{
	...
	for (pNetdevEntry = &__NETDEVTAB__[0]; pNetdevEntry != &__NETDEVTAB_END__; pNetdevEntry++)
	{
        ...
        {
        	//diag_printf("\ni1\n");
	        if(pNetdevEntry->status == CYG_NETDEVTAB_STATUS_AVAIL)
				return true;
			else 
			{
			 	if (pNetdevEntry->init(pNetdevEntry)) {
		            pNetdevEntry->status = CYG_NETDEVTAB_STATUS_AVAIL;
					//diag_printf("i2\n");
					return true;
		        } else {
		            // What to do if device init fails?
		            //diag_printf("i3\n");
		            pNetdevEntry->status = 0;  // Device not [currently] available
		            return false;
		        }
			}	
        }
...
static bool rti_ecos_mbss_init(struct cyg_netdevtab_entry *tab)
{
	...
	pNetDev = (struct eth_drv_sc *)tab->device_instance;
	sprintf(devName, "%s0", SECOND_INF_MAIN_DEV_NAME);
	rti_ecos_main_dev_init(devName);
