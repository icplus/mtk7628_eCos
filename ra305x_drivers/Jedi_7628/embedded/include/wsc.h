...
#define WSC_HDR_BTN_MR_PRESS_FLG_GET(__pAd, __FlgIsPressed)			\
	{									\
		UINT32 __gpio_value, __gpio_pol;   \
            __gpio_value = HAL_REG32(RTMP_PIO_CTL_ADDR + RTMP_PIO6332_DIR_OFFSET); \
		    __gpio_value &= ~(1<<6); 						\
            HAL_REG32(RTMP_PIO_CTL_ADDR + RTMP_PIO6332_DIR_OFFSET) = __gpio_value; \
            __gpio_pol = HAL_REG32(RTMP_PIO_CTL_ADDR + RTMP_PIO6332_POL_OFFSET); \
            __gpio_value = HAL_REG32(RTMP_PIO_CTL_ADDR + RTMP_PIO6332_DATA_OFFSET); \
		__FlgIsPressed = ((~__gpio_value) & (1<<6))?1:0;					\
	}

#define GPIO_MODE_REG	0xB0000060
#define GPIO_MODE_I2C	(0x01 << 20)
/* GPIO 5 */
#define SIGNAL_BTN_MR_PRESS_FLG_GET(__pAd, __FlgIsPressed)			\
	{									\
		UINT32 __gpio_mode; \
		__gpio_mode = HAL_REG32(GPIO_MODE_REG); \
		__gpio_mode |= GPIO_MODE_I2C; \
		HAL_REG32(GPIO_MODE_REG) = __gpio_mode; \
		                                        \
		UINT32 __gpio_value, __gpio_pol;   \
            __gpio_value = HAL_REG32(RTMP_PIO_CTL_ADDR + RTMP_PIO3100_DIR_OFFSET); \
		    __gpio_value &= ~(1<<5); 						\
            HAL_REG32(RTMP_PIO_CTL_ADDR + RTMP_PIO3100_DIR_OFFSET) = __gpio_value; \
            __gpio_pol = HAL_REG32(RTMP_PIO_CTL_ADDR + RTMP_PIO3100_POL_OFFSET); \
            __gpio_value = HAL_REG32(RTMP_PIO_CTL_ADDR + RTMP_PIO3100_DATA_OFFSET); \
		__FlgIsPressed = ((~__gpio_value) & (1<<5))?1:0;					\
	}
#endif
#else
