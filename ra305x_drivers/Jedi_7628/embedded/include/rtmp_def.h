...
#define ST_STR(_st) \
	case _st: \
		return #_st; 
static inline const char *apcli_sync_sm_str(unsigned int state)
{
	switch (state) {
		ST_STR(APCLI_MT2_MLME_PROBE_REQ)
		ST_STR(APCLI_MT2_PEER_PROBE_RSP)
		ST_STR(APCLI_MT2_PEER_BEACON)
		ST_STR(APCLI_MT2_PROBE_TIMEOUT)
		ST_STR(APCLI_MAX_SYNC_MSG)
	}
}

static inline const char *apcli_ctrl_sm_str(unsigned int state)
{
	switch (state) {
		ST_STR(APCLI_CTRL_JOIN_REQ)
		ST_STR(APCLI_CTRL_PROBE_RSP)
		ST_STR(APCLI_CTRL_AUTH_RSP)
		ST_STR(APCLI_CTRL_DISCONNECT_REQ)
		ST_STR(APCLI_CTRL_PEER_DISCONNECT_REQ)
		ST_STR(APCLI_CTRL_ASSOC_RSP)
		ST_STR(APCLI_CTRL_DEASSOC_RSP)
		ST_STR(APCLI_CTRL_JOIN_REQ_TIMEOUT)
		ST_STR(APCLI_CTRL_AUTH_REQ_TIMEOUT)
		ST_STR(APCLI_CTRL_ASSOC_REQ_TIMEOUT)
		ST_STR(APCLI_CTRL_MT2_AUTH_REQ)
		ST_STR(APCLI_CTRL_MT2_ASSOC_REQ)
		ST_STR(APCLI_CTRL_SCAN_DONE)
		ST_STR(APCLI_MIC_FAILURE_REPORT_FRAME)
	}
}

static inline const char *apcli_sm_str(unsigned int mach, unsigned int state)
{
	const char *str = NULL;
	
	switch (mach) {
		case APCLI_SYNC_STATE_MACHINE:
			str = apcli_sync_sm_str(state);
			break;
		case APCLI_CTRL_STATE_MACHINE:
			str = apcli_ctrl_sm_str(state);
			break;
		default:
			break;
	}
