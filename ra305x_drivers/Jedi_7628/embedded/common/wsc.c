...
#define STORE_SCAN_INFO		50
typedef struct __wifi_info
{
	char site_chnl[4];
	char site_ssid[65];
	char site_ssid_raw[65];
	char site_bssid[24];
	char site_sec[24];
	char site_signal[10];
}wifi_info;
wifi_info scan_info[STORE_SCAN_INFO] = {0};
...
static void  do_site_survey(PRTMP_ADAPTER	pAd, char *buff, int buff_len)
{
	RTMP_IOCTL_INPUT_STRUCT	wrq = {0};
	//unsigned char buff[IW_SCAN_MAX_DATA] = {0}; // error using function;

	memset(buff, 0x0, buff_len);
	wrq.u.data.pointer = (caddr_t) buff;
	wrq.u.data.length  = buff_len; //8192Byte; 8K
	wrq.u.data.flags|= (RTPRIV_IOCTL_FLAG_NOSPACE | RTPRIV_IOCTL_FLAG_NODUMPMSG);

	RTMPIoctlGetSiteSurvey(pAd, &wrq);
}

unsigned int failed_try_time =0;
static int parse_site_survey(PRTMP_ADAPTER	pAd, char *res, int res_len, const char *target_ssid, int target_ssid_len)
{
	char *sp, *op;
	char site_chnl[4] = {0};
	char site_ssid[65] = {0};
	...
	while (*sp && ((len - (sp - op)) >= 0))
	{
		...
		printf("tmp = %s\n", tmp);
#else
		//snprintf(scan_info[i].site_chnl, sizeof(site_chnl), trim_r(site_chnl));
		snprintf(scan_info[i].site_ssid, sizeof(site_ssid), trim_r(site_ssid));
		//snprintf(scan_info[i].site_ssid_raw, sizeof(site_ssid_raw), trim_r(site_ssid_raw));
		//snprintf(scan_info[i].site_bssid, sizeof(site_bssid), trim_r(site_bssid));
		//snprintf(scan_info[i].site_sec, sizeof(site_sec), trim_r(site_sec));
		snprintf(scan_info[i].site_signal, sizeof(site_signal), trim_r(site_signal));

		ssid_len = strlen(scan_info[i].site_ssid);
		if( (ssid_len == target_ssid_len)  && !strncmp(target_ssid, scan_info[i].site_ssid, target_ssid_len) ) // min(ssid_len, target_ssid_len)
		{
			printf("\n\nhad  found  target SSID \n");
			//printf("target_ssid=%s, target_ssid_len=%d\n", target_ssid, target_ssid_len);
			//printf("scan_info[%d].site_ssid=%s, ssid_len=%d\n", i, scan_info[i].site_ssid, ssid_len);

			tmp_signal = atoi(scan_info[i].site_signal);
			// control led on/off
			if(tmp_signal >= 90)
			{
				printf("signal = %d\n", tmp_signal);
			}
			else if(tmp_signal >= 70)
			{
				printf("signal = %d\n", tmp_signal);
			}
			else if(tmp_signal >= 50)
			{
				printf("signal = %d\n", tmp_signal);
			}
			else if(tmp_signal >= 30)
			{
				printf("signal = %d\n", tmp_signal);
			}
			else
	...
	// if not found target SSID
	printf("target_ssid=%s, target_ssid_len=%d  not found ssid signal, try again.\n", target_ssid, target_ssid_len);
	failed_try_time++;
	if(failed_try_time < 5)
	{
		do_site_survey(pAd, res, res_len);
	}
	else
		failed_try_time = 0;
	return -1;
...
/* WARNING: The "buff" with IW_SCAN_MAX_DATA size in length is too large to fit in the function stack */
unsigned char buff[IW_SCAN_MAX_DATA] = {0};
	...
	IN	PRTMP_ADAPTER	pAd)
{
 	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	BOOLEAN flg_pressed = 0;
	 BOOLEAN signal_button_pressed =  1;
	static char pbc_button_press_start =0;
	int ret = -1;
	

// add by Shawn
#if 0 //#ifdef APCLI_SUPPORT

	UCHAR ifIndex = 0; //apcli0
	BOOLEAN apcliEn;
	NDIS_802_11_SSID target_ssid;
	static int num = 0;
	int  len;

	apcliEn = pAd->ApCfg.ApCliTab[ifIndex].Enable;
	len = pAd->ApCfg.ApCliTab[ifIndex].CfgSsidLen ;

	SIGNAL_BTN_MR_PRESS_FLG_GET(__pAd, signal_button_pressed);

	if ( signal_button_pressed == 1)
	{
		printf("signal button pressed\n");
		if(!scan_flag)
		{
			if( apcliEn == TRUE && len != 0)
			{
				//ApSiteSurvey(pAd, &target_ssid, SCAN_ACTIVE, FALSE); // target ssid
	...
	if(scan_flag)
	{
		...
		if(num >= 20)
		{
			do_site_survey(pAd, buff, sizeof(buff));
#if 0
			//只能当个字符打印
			int i;
			for(i = 0; i < wrq.u.data.length; i++)
				diag_printf("%c",buff[i]);
#endif
			//printf("===> SSID=%s\n", pAd->ApCfg.ApCliTab[ifIndex].CfgSsid);
			NdisZeroMemory(&target_ssid, sizeof(NDIS_802_11_SSID));
			memcpy(target_ssid.Ssid, pAd->ApCfg.ApCliTab[ifIndex].CfgSsid, len);
			target_ssid.SsidLength = len;

			// handle  wifi  scan data
			ret = parse_site_survey(pAd, buff, sizeof(buff), target_ssid.Ssid, target_ssid.SsidLength);
			if(!ret)
			{
				scan_flag=0;
				printf("Reset scan_flag\n");
			}
			num=0;
		}
	}
...
#if 0
/*
 取消了WPS功能, 这里只按键只有 reset;
 #if 0; 现在reset功能也没有, 按键定义在ra305x_ap_adv/ra305x_router/init/boards.c
*/
	WSC_HDR_BTN_MR_PRESS_FLG_GET(pAd, flg_pressed);
