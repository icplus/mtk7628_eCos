	...
	IN  UINT32 MsgLen)
{
	...
	/* Ensure it is large enough for those un-printable SSID */
	CHAR		Ssid[MAX_LEN_OF_SSID * 2 +1] = {0};
	...
	else
	{
		...
		sprintf(Ssid, "0x");
		/* FIXME: use sizeof(Ssid) instead */
		for (idx = 0; ((2 + idx * 2) < MAX_LEN_OF_SSID * 2) && (idx < pBss->SsidLen); idx++)
...
#ifdef AIRPLAY_SUPPORT
	sprintf(msg+strlen(msg),"%-4s%-65s%-4s%-20s%-23s%-9s%-7s%-7s%-3s\n",
