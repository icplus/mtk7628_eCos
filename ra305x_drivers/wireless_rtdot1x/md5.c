...
void md5_mac(UCHAR *key, ULONG key_len, UCHAR *data, ULONG data_len, UCHAR *mac)
{
	...
	Dot1xMD5Update(&context,	key, key_len);
	Dot1xMD5Update(&context,	data, data_len);
	Dot1xMD5Update(&context,	key, key_len);
	Dot1xMD5Final(mac, &context);
}
 ...
 */
void Dot1xhmac_md5(UCHAR *key, ULONG key_len, UCHAR *data, ULONG data_len, UCHAR *mac)
{
	MD5_CTX	context;
	...
	if (key_len	> 64) {
		...
		Dot1xMD5Init(&ttcontext);
		Dot1xMD5Update(&ttcontext, key, key_len);
		Dot1xMD5Final(tk, &ttcontext);
	...
	/* perform inner MD5 */
	Dot1xMD5Init(&context);					 /*	init context for 1st pass */
	Dot1xMD5Update(&context,	k_ipad,	64);	 /*	start with inner pad */
	Dot1xMD5Update(&context,	data, data_len); /*	then text of datagram */
	Dot1xMD5Final(mac, &context);			 /*	finish up 1st pass */

	/* perform outer MD5 */
	Dot1xMD5Init(&context);					 /*	init context for 2nd pass */
	Dot1xMD5Update(&context,	k_opad,	64);	 /*	start with outer pad */
	Dot1xMD5Update(&context,	mac, 16);		 /*	then results of	1st	hash */
	Dot1xMD5Final(mac, &context);			 /*	finish up 2nd pass */
}
 ...
 *      None	    
 */
VOID Dot1xMD5Init(MD5_CTX *pCtx)
{
 ...
 */
VOID Dot1xMD5Update(MD5_CTX *pCtx, UCHAR *pData, ULONG LenInBytes)
{
 ...
 */
VOID Dot1xMD5Final(UCHAR Digest[16], MD5_CTX *pCtx)
{
    UCHAR Remainder;
    UCHAR PadLenInBytes;
    UCHAR *pAppend=0;
