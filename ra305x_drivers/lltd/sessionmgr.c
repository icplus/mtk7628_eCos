...
{
    IF_TRACED(TRC_STATE)
        diag_printf("smS (Nascent): Entered with event %s",smEvent_names[evt->evtType]);
...
/**/            DEBUG({puts("is acking me....");})
                evt->isAckingMe = TRUE;
                IF_TRACED(TRC_STATE)
                    diag_printf("smS[%X] (Nascent): Leaving for Complete.\n", (uint)evt->ssn);
                END_TRACE
...
/**/            DEBUG({puts("is NOT acking me....");})
                ...
                IF_TRACED(TRC_STATE)
                    diag_printf("smS[%X] (Nascent): Leaving for Pending.\n", (uint)evt->ssn);
                END_TRACE
      ...
      default :
	  	__asm__("KH1:");
        IF_TRACED(TRC_STATE)
            diag_printf("smS: Nascent handler ignored event %s\n",smEvent_names[evt->evtType]);
        END_TRACE
        break;
    }
...
{
    IF_TRACED(TRC_STATE)
        diag_printf("smS[%p%s] (Pending): Entered with event %s", evt->ssn,
               (evt->ssn == g_topo_session)?"-T":"", smEvent_names[evt->evtType]);
	
			diag_printf("KH:g_this_event <--\n");
		
       //if (g_this_event.evtType==evtPacketRcvd)
       if(evt->evtType==evtPacketRcvd)
        {
            diag_printf(" (%s)\n",Topo_opcode_names[g_opcode]);
        } else {
            puts("");
...
//*/        DEBUG({puts("is acking me....");})
            ...
            if (g_sequencenum  &&  evt->ssn->ssn_XID != g_sequencenum  &&  g_topo_session == evt->ssn)
            {
                ...
                IF_TRACED(TRC_PACKET)
                    diag_printf("smS (Pending): Discover-ack, new XID (%d) != old XID (%d); resetting topo session....\n",
                ...
                IF_TRACED(TRC_PACKET)
                    diag_printf("smS (Pending): After topo reset, re-creating the topo-session ptr....\n");
                END_TRACE
                g_topo_session = evt->ssn;
            ...
            IF_TRACED(TRC_STATE)
                diag_printf("smS[%X] (Pending): Leaving for Complete.\n", (uint)evt->ssn);
            END_TRACE
            evt->ssn->ssn_state = smS_Complete;
            restart_inactivity_timer((evt->ssn->ssn_TypeOfSvc == ToS_TopologyDiscovery) ? TOPO_CMD_ACTIVITYTIMEOUT : TOPO_GENERAL_ACTIVITYTIMEOUT);
        } else {
            ...
            if (g_sequencenum  &&  evt->ssn->ssn_XID != g_sequencenum  &&  g_topo_session == evt->ssn)
            {
                ...
                IF_TRACED(TRC_PACKET)
                    diag_printf("smS (Pending): Discover-noack, new XID (%d) != old XID (%d); resetting topo session....\n",
                           g_sequencenum, evt->ssn->ssn_XID);
                END_TRACE
                g_this_event.evtType = evtResetRcvd;
                g_this_event.isInternalEvt = TRUE;
      ...
      case evtResetRcvd:
        ...
        if (evt->ssn == g_topo_session  &&  g_topo_session->ssn_is_valid == TRUE)
        {
            ...
            for (i=0; i < MAX_NUM_SESSIONS; ssn++, i++)
            {
                ...
                {
                    IF_TRACED(TRC_STATE)
                        diag_printf("smS[%X] (Temporary): Leaving for Nascent.\n", (uint)ssn);
                    END_TRACE
        ...
        IF_TRACED(TRC_STATE)
            diag_printf("smS[%X] (Pending): Leaving for Nascent.\n", (uint)evt->ssn);
        END_TRACE
		if(evt->ssn == NULL)
			diag_printf("SSN is null\n");
      ...
      case evtHelloDelayTimeout:
		if(evt->ssn == NULL)
			diag_printf("Hello Delay ssn is NULL\n");
	...
	break;
      ...
      default :
        IF_TRACED(TRC_STATE)
            diag_printf("smS (Pending): Ignored event %s\n", smEvent_names[evt->evtType]);
        END_TRACE
        break;
    }
    return PROCESSING_COMPLETED;
...
{
    IF_TRACED(TRC_STATE)
        diag_printf("smS (Temporary): Entered with event %s",smEvent_names[evt->evtType]);
        if (g_this_event.evtType==evtPacketRcvd)
        {
            diag_printf(" (%s)\n",Topo_opcode_names[g_opcode]);
        } else {
    ...
    switch (evt->evtType)
    {
      ...
      default:
        IF_TRACED(TRC_STATE)
            diag_printf("smS (Temporary): Ignored event %s\n",smEvent_names[evt->evtType]);
        END_TRACE
        break;
    }
...
{
    IF_TRACED(TRC_STATE)
        diag_printf("smS (Complete): Entered with event %s",smEvent_names[evt->evtType]);
        if (g_this_event.evtType==evtPacketRcvd)
        {
            diag_printf(" (%s)\n",Topo_opcode_names[g_opcode]);
        } else {
...
//!!            osl_set_arprx(FALSE, ts);
        ...
        /* In any case, reset (invalidate) this session */
        IF_TRACED(TRC_STATE)
            diag_printf("smS[%X] (Complete): Leaving for Nascent.\n", (uint)evt->ssn);
        END_TRACE
        evt->ssn->ssn_state = smS_Nascent;
        CANCEL(evt->ssn->ssn_InactivityTimer);
        evt->ssn->ssn_is_valid = FALSE;
...
//*/        DEBUG({puts("is acking me....");})
            ...
            if (g_sequencenum  &&  evt->ssn->ssn_XID != g_sequencenum  &&  g_topo_session == evt->ssn)
            {
                ...
                IF_TRACED(TRC_PACKET)
                    diag_printf("smS (Complete): Discover-ack, new XID (%d) != old XID (%d); resetting topo session....\n",
                           g_sequencenum, evt->ssn->ssn_XID);
                END_TRACE
                g_this_event.evtType = evtResetRcvd;
                g_this_event.isInternalEvt = TRUE;
                ...
                g_this_event.isInternalEvt = isActuallyInternal;
                IF_TRACED(TRC_PACKET)
                    diag_printf("smS (Complete): After topo reset, re-creating the topo-session ptr....\n");
                END_TRACE
                g_topo_session = evt->ssn;
                /* Regardless of whether this is acking or not, we must accept the new XID, unless it's zero */
                evt->ssn->ssn_XID = g_sequencenum;
            ...
            if (g_sequencenum  &&  evt->ssn->ssn_XID != g_sequencenum  &&  g_topo_session == evt->ssn)
            {
                ...
                IF_TRACED(TRC_PACKET)
                    diag_printf("smS (Complete): Discover-noack, new XID (%d) != old XID (%d); resetting topo session....\n",
                           g_sequencenum, evt->ssn->ssn_XID);
                END_TRACE
                g_this_event.evtType = evtResetRcvd;
                g_this_event.isInternalEvt = TRUE;
                ...
                evt->ssn->ssn_XID = g_sequencenum;
                IF_TRACED(TRC_STATE)
                    diag_printf("smS[%X] (Complete): Leaving for Pending (No-ack, changed XID).\n", (uint)evt->ssn);
                END_TRACE
                evt->ssn->ssn_state = smS_Pending;
                restart_inactivity_timer(TOPO_HELLO_ACTIVITYTIMEOUT);
            } else {
      ...
      default :
        IF_TRACED(TRC_STATE)
//*/            diag_printf("smS (Complete): Ignored event %s\n",smEvent_names[evt->evtType]);
        END_TRACE
        break;
    }
...
{
    IF_TRACED(TRC_STATE)
	diag_printf("smS_process_event: Entered with event %s\n",smEvent_names[evt->evtType]);
    END_TRACE

    if (evt->ssn == NULL)
    {
        IF_TRACED(TRC_STATE)
            diag_printf("smS (no-state): Not dispatching - no session associated with event %s",
                   smEvent_names[evt->evtType]);
            if (g_this_event.evtType==evtPacketRcvd)
            {
                diag_printf(" (%s)\n",Topo_opcode_names[g_opcode]);
