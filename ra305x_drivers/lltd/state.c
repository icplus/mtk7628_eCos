...
find_session(etheraddr_t *this_addr)
{
    ...
    for (i=0; i < MAX_NUM_SESSIONS; ssn++, i++)
    {
        ...
        {
//*/        DEBUG({diag_printf("find_session returning session %d @ %X\n",i,(uint)ssn);})
            return ssn;
        }
    }
...
state_process_packet()
{
    ...
    IF_TRACED((TRC_STATE|TRC_PACKET))
        diag_printf("state_process_packet: Entered with event %s",smEvent_names[g_this_event.evtType]);
    ...
    g_this_event.ssn = this_session;
#if 0
  protocol_event_t g_this_event_tmp;
   char *p=&g_this_event_tmp;
   char *p1=&g_this_event;
  memset(p,0x00,sizeof(protocol_event_t));
  memcpy(p, p1, sizeof(protocol_event_t));
    smStatus = smS_process_event( &g_this_event_tmp );
diag_printf("smS_process_event:done with event\n");
    if (smStatus != PROCESSING_ABORTED)
    {
        smStatus = smE_process_event( &g_this_event_tmp );
		        diag_printf("smE_process_event:done with event\n");

    }

    if (smStatus != PROCESSING_ABORTED)
    {
        smStatus = smT_process_event( &g_this_event_tmp );
		diag_printf("smT_process_event:done with event\n");
    }
	memcpy(p1, p, sizeof(protocol_event_t));
 #else
    ...
    IF_TRACED(TRC_PACKET)
        diag_printf("state_process_packet: Leaving - done with event %s\n",smEvent_names[g_this_event.evtType]);
    END_TRACE
    return 0;	/* Success! */
}
...
state_process_timeout()
{
    ...
    IF_TRACED(TRC_STATE)
        if (g_this_event.evtType!=evtBlockTimeout)
			diag_printf("Enter state_process_timeout\n");
    END_TRACE
#endif
    g_rcvd_pkt_len = 0;
 ...
 IF_TRACED(TRC_STATE)
        if (g_this_event.evtType!=evtBlockTimeout)
			diag_printf("Exit state_process_timeout\n");
    END_TRACE
    return 0;	/* Success! */
}
