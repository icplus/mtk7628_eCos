	 ...
	 topo_opcode_t g_opcode, uint16_t seqnum, bool_t use_broadcast)
{
    ...
    IF_TRACED(TRC_PACKET)
        diag_printf("fmt_base: src=" ETHERADDR_FMT "\n", ETHERADDR_PRINT(srchw));
        diag_printf("fmt_base: dst=" ETHERADDR_FMT "\n", ETHERADDR_PRINT(dsthw));
        diag_printf("fmt_base: tos=%d g_opcode=%d seq=%d bcast=%s\n", \
...
//#define BYTE_DUMP
	...
	diag_printf("sending " FMT_SIZET " bytes:\n",nbytes);
	for (i=0; i<j; i++)
	{
	    diag_printf("%02x%s%s", buf[i], (i%2)==1?" ": "", (i%16)==15? "\n": "");
	}
	diag_printf("\n");
    }
#endif
...
packetio_tx_flat(void)
{
    ...
    IF_TRACED(TRC_PACKET)
	dbgprintf("tx_flat(len:" FMT_SIZET "+" FMT_SIZET "+" FMT_SIZET "): " FMT_UINT32 ":" FMT_UINT32 " bytes, " FMT_UINT32 \
	          ":" FMT_UINT32 " packets -> " ETHERADDR_FMT "\n",
                  sizeof(topo_ether_header_t),sizeof(topo_base_header_t),sizeof(topo_flat_header_t),
		  g_ctc_bytes,fh->fh_ctc_bytes, g_ctc_packets,fh->fh_ctc_packets,
		  ETHERADDR_PRINT(&g_topo_session->ssn_mapper_real));
    END_TRACE
}
...
packetio_tx_emitee(topo_emitee_desc_t *ed)
{
    ...
    IF_TRACED(TRC_PACKET)
        diag_printf("tx_emitee: src=" ETHERADDR_FMT "\n", ETHERADDR_PRINT(&ehdr->eh_src));
        diag_printf("tx_emitee: dst=" ETHERADDR_FMT "\n", ETHERADDR_PRINT(&ehdr->eh_dst));
        diag_printf("tx_emitee: real-src=" ETHERADDR_FMT "\n", ETHERADDR_PRINT(&bhdr->tbh_realsrc));
        diag_printf("tx_emitee: tos=%d g_opcode=%d seq=%d bcast=%s\n", bhdr->tbh_tos, Opcode, bhdr->tbh_seqnum, "false");
    END_TRACE
...
void
packetio_recv_handler(struct ifnet *ifp, unsigned char *inpkt, int len)
{
	LLTD_CMD_STRUCT *msg;
//	char * buf;
	...
	if (lltd_msgbox_id == -1)
       ...
       msg = malloc(sizeof(LLTD_CMD_STRUCT));
       if (msg == 0)
	{
		diag_printf("Msg malloc is fail\n");
		return;
        }
...
//       if (buf == 0)
//	{
//		diag_printf("Msg Buffer is fail\n");
//		free(msg);
//		return;
//        }
//	  memcpy(buf, inpkt, len);
	 ...
	 msg->len = len;
        ...
        if (cyg_mbox_put((cyg_handle_t)lltd_msgbox_id, msg) == 0)
        {
//        	free(buf);
        	free(msg);    
		diag_printf("lltd put msg failed\n");
        }
        return;
}
void lltd_packetio_recv_handler(struct ifnet *ifp, unsigned char *inpkt, int len)
{
    uint16_t                            thisSeqnum;
	
    enum packet_verification_results	pktValidity;
    memset(g_rxbuf,0x00,RXBUFSZ);
	memcpy(g_rxbuf,inpkt,len);
    g_rcvd_pkt_len=len;
    if (g_rcvd_pkt_len <= 0)
