...
smE_QuiescentHandler( protocol_event_t* evt )
{
    IF_TRACED(TRC_STATE)
        ...
        {
            diag_printf("smE (Quiescent): Entered with event %s",smEvent_names[evt->evtType]);
    ...
    switch (evt->evtType)
    {
      case evtDiscoverRcvd:
        if (evt->isNewSession)
        {
        	if(g_this_event.ssn==NULL)
				diag_printf("Dis_SSN is NULL\n");
            if (g_this_event.ssn->ssn_state == smS_Complete)
            {
                CANCEL(g_block_timer);
                g_smE_state = smE_Wait;
                IF_DEBUG
                    diag_printf("smE (Quiescent): Leaving for Wait state\n");
                END_DEBUG
            } else {
                band_InitStats();
...
//!!            osl_set_arprx(g_osl, TRUE);
                ...
                IF_DEBUG
                    diag_printf("smE (Quiescent):  Leaving for Pausing state\n");
                END_DEBUG
            }
        } else {
      ...
      default :
        IF_DEBUG
//*/        diag_printf("smE (Quiescent): Ignored event %s\n",smEvent_names[evt->evtType]);
        END_DEBUG
        break;
    }
...
smE_PausingHandler( protocol_event_t* evt )
{
    IF_TRACED(TRC_STATE)
        if (evt->evtType != evtBlockTimeout)
        {
            diag_printf("smE[%p%s] (Pausing): Entered with event %s", evt->ssn,
    ...
    switch (evt->evtType)
    {
      ...
      case evtHelloDelayTimeout:
	    ...
	    g_smE_state = smE_Wait;
	    IF_DEBUG
              diag_printf("smE (Pausing): Leaving for Wait state\n");
	    END_DEBUG
	}
        break;
      ...
      case evtDiscoverRcvd:
        ...
        if (OnlyCompleteSessions()==TRUE)
        {
            ...
            g_smE_state = smE_Wait;
            IF_DEBUG
                diag_printf("smE (Pausing): Leaving for Wait state (Only-completed-sessions arc)\n");
            END_DEBUG
        }
        break;
      ...
      case evtResetRcvd:
        ...
        if (SessionTableIsEmpty()==TRUE)
        {
            ...
            CANCEL(g_hello_timer);
            IF_DEBUG
                diag_printf("smE (Pausing): Killing the Block- and Hello-timers after getting reset.\n");
                diag_printf("smE (Pausing): Leaving for Quiescent state\n");
            END_DEBUG
      ...
      default :
        IF_DEBUG
//*/        diag_printf("smE: Pausing state got ignored event %s\n",smEvent_names[evt->evtType]);
        END_DEBUG
        break;
    }
...
smE_WaitHandler( protocol_event_t* evt )
{
    IF_TRACED(TRC_STATE)
        if (evt->evtType != evtBlockTimeout)
        {
            diag_printf("smE (Wait): Entered with event %s",smEvent_names[evt->evtType]);
            if (g_this_event.evtType==evtPacketRcvd)
            {
                diag_printf(" (%s)\n",Topo_opcode_names[g_opcode]);
            } else {
    ...
    switch (evt->evtType)
    {
      case evtDiscoverRcvd:
        ...
        if (evt->isNewSession)
        {
            if (evt->ssn->ssn_state!=smS_Complete)
            {
                ...
                g_smE_state = smE_Pausing;
                IF_DEBUG
                    diag_printf("smE (Wait): Leaving for Pausing state\n");
                END_DEBUG
            } else {
                /* just stay here */
            }
      ...
      case evtResetRcvd:
        ...
        if (SessionTableIsEmpty()==TRUE)
        {
            g_smE_state = smE_Quiescent;
            IF_DEBUG
                diag_printf("smE (Wait): Leaving for Quiescent state\n");
            END_DEBUG
        }
        break;
      ...
      default :
        IF_DEBUG
//*/            diag_printf("smE (Wait): Ignored event %s\n",smEvent_names[evt->evtType]);
        END_DEBUG
        break;
    }
...
{
    IF_DEBUG
	//diag_printf("smE_process_event: Entered with event %s\n",smEvent_names[evt->evtType]);
    END_DEBUG
    switch (g_smE_state)
    {
      case smE_Quiescent:
