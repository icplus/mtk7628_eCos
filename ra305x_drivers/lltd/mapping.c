...
ChargeConsume(int pktlen)
{
    ...
    gettimeofday(&now, NULL);
    timeval_add_ms(&now, TOPO_CHARGE_TIMEOUT/*-100*/);
    CANCEL(g_charge_timer);
    g_charge_timer = event_add(&now, state_charge_timeout, /*state:*/NULL);
}
...
smT_QuiescentHandler( protocol_event_t* evt )
{
    IF_TRACED(TRC_STATE)
        ...
        {
            diag_printf("smT (Quiescent): Entered with event %s",smEvent_names[evt->evtType]);
    ...
    switch (evt->evtType)
    {
        ...
        switch (g_opcode)
        {
          ...
          case Opcode_QueryLargeTlv:
            IF_TRACED(TRC_PACKET)
                diag_printf("smT (Quiescent): Hoisting packet ignored.\n");
            END_TRACE
            break;
          ...
          case Opcode_QueryLargeTlvResp:
            IF_TRACED(TRC_STATE)
                diag_printf("smT (Quiescent): Ignored packet w/ known opcode: %s\n",
                       Topo_opcode_names[g_opcode]);
            END_TRACE
            break;
      ...
      case evtDiscoverRcvd:
        if (evt->isAckingMe)
        {
            ...
            IF_TRACED(TRC_STATE)
                diag_printf("smT (Quiescent): Leaving for Command state\n");
            END_TRACE
      ...
      case evtEmitRcvd:
        IF_TRACED(TRC_PACKET)
            diag_printf("smT (Quiescent): Hoisting packet (Emit) ignored.\n");
        END_TRACE
        break;
      ...
      default:
        IF_DEBUG
            diag_printf("smT (Quiescent): Ignored event %s\n",smEvent_names[evt->evtType]);
        END_DEBUG
        break;
...
smT_CommandHandler( protocol_event_t* evt )
{
    IF_TRACED(TRC_STATE)
        if (evt->evtType != evtBlockTimeout)
        {
            diag_printf("smT (Command): Entered with event %s",smEvent_names[evt->evtType]);
            if (g_this_event.evtType==evtPacketRcvd)
            {
                diag_printf(" (%s)\n",Topo_opcode_names[g_opcode]);
            } else {
    ...
    switch (evt->evtType)
    {
      case evtPacketRcvd:
        switch (g_opcode)
        {
          ...
          case Opcode_Probe:
            IF_TRACED(TRC_PACKET)
                diag_printf("smT (Command): Logging Probe from " ETHERADDR_FMT "\n",ETHERADDR_PRINT(&g_base_hdr->tbh_realsrc));
            END_TRACE
          ...
          case Opcode_Query:
            IF_TRACED(TRC_PACKET)
                diag_printf("smT (Command): Sending query-Resp\n");
            END_TRACE
          ...
          case Opcode_QueryLargeTlv:
            {
                ...
                for (;tlvDescr->number != 0; tlvDescr++)
                {
                    if (tlvDescr->number == g_qltlv_hdr->qh_type)
                    {
                        ...
                        IF_TRACED(TRC_PACKET)
                            diag_printf("smT (Command): Sending qltlv-Resp with LTLV # 0x%X\n", tlvDescr->number);
                        END_TRACE
          ...
          case Opcode_Flat:
            IF_TRACED(TRC_PACKET)
                diag_printf("smT (Command): Ignored packet w/ known opcode: %s\n",Topo_opcode_names[g_opcode]);
            END_TRACE
            break;

          default:
            IF_TRACED(TRC_PACKET)
                diag_printf("smT (Command): Ignored packet w/ unknown opcode: %d\n",g_opcode);
            END_TRACE
            return PROCESSING_ABORTED;
        }   /*** end of switch (g_opcode) ***/
        break;
      ...
      case evtResetRcvd:
        ...
        IF_TRACED(TRC_PACKET)
            diag_printf("smT (Command): ResetRcvd, %s for topo-session\n",evt->ssn == g_topo_session?"is":"is not");
        END_TRACE
        if (evt->ssn == g_topo_session)
        {
            ...
            IF_TRACED(TRC_PACKET)
                diag_printf("smT (Command): Leaving for Quiescent (Inactive or Reset).\n");
            END_TRACE
      ...
      case evtEmitRcvd:
        ...
        if (SufficientChargeAvailable())
        {
            ...
            ProcessEmits();
            DEBUG({diag_printf("smT (Command): g_emit_remaining = %d\n",g_emit_remaining);})
            if (g_emit_remaining > 0)
            {
                bool_t  pausing;

                IF_TRACED(TRC_STATE)
                    diag_printf("smT (Command): Leaving for Emit\n");
                END_TRACE
                ...
                if (pausing!=TRUE)
                {
                    ...
                    IF_TRACED(TRC_STATE)
                        diag_printf("smT (Command): No Pause in 1st emitee: Inject an EmitTimeout immediately.\n");
                    END_TRACE
                    return KEEP_GOING;
                }
            } else {
      ...
      default :
        IF_TRACED(TRC_STATE)
            diag_printf("smT (Command): Ignored event %s\n",smEvent_names[evt->evtType]);
        END_TRACE
        break;
    }
...
{
    IF_TRACED(TRC_STATE)
        diag_printf("smT (Emit): Entered with event %s",smEvent_names[evt->evtType]);
        if (g_this_event.evtType==evtPacketRcvd)
        {
            diag_printf(" (%s)\n",Topo_opcode_names[g_opcode]);
        } else {
    ...
    switch (evt->evtType)
    {
      case evtPacketRcvd:
        switch (g_opcode)
        {
          ...
          case Opcode_QueryLargeTlvResp:
            IF_TRACED(TRC_PACKET)
                diag_printf("smT (Emit): Ignored packet w/ known opcode: %s\n",Topo_opcode_names[g_opcode]);
            END_TRACE
            break;

          default:
            IF_TRACED(TRC_PACKET)
                diag_printf("smT (Emit): Ignored packet w/ unknown opcode: %d\n",g_opcode);
            END_TRACE
            return PROCESSING_ABORTED;
        }   /*** end of switch (g_opcode) ***/
        break;
      ...
      case evtResetRcvd:
        ...
        if (evt->ssn == g_topo_session)
        {
            ...
            /* and return to Quiescent state. */
            IF_TRACED(TRC_STATE)
                diag_printf("smT (Emit): Leaving for Quiescent (Reset or Inactive)\n");
            END_TRACE
            g_smT_state = smT_Quiescent;
        }
        break;
      ...
      case evtEmitTimeout:
        ...
        if (g_emit_remaining == 0)
        {
            ...
            IF_TRACED(TRC_STATE)
                diag_printf("smT (Emit): Leaving for Command\n");
            END_TRACE
            g_smT_state = smT_Command;
            CANCEL(g_emit_timer);
            if (g_emit_seqnum)
      ...
      default :
        IF_TRACED(TRC_STATE)
            diag_printf("smT (Emit): Ignored event %s\n",smEvent_names[evt->evtType]);
        END_TRACE
        break;
    }
...
#ifdef __DEBUG__
    IF_DEBUG
//*/    diag_printf("smT_process_event: Entered with event %s\n",smEvent_names[evt->evtType]);
    END_DEBUG
#endif
...
#ifdef __DEBUG__
        IF_DEBUG
//*/        diag_printf("smT_process_event: Processing event %s\n",smEvent_names[evt->evtType]);
        END_DEBUG
#endif
        switch (g_smT_state)
        {
