...
int opts;
{
	...
	memcpy((char *)&ips, (u_long)sp, sizeof(ips));

	PRINTF("%s -> ", iphostname(ips.is_v, &ips.is_src.in4));
	PRINTF("%s ttl %ld pass %#x pr %d state %d/%d\n",
		iphostname(ips.is_v, &ips.is_dst.in4),
...
(__FreeBSD_version >= 220000) || defined(__OpenBSD__)
	...
	PRINTF("\tpkt_security & %x = %x, pkt_auth & %x = %x\n",
		...
		ips.is_auth);
	PRINTF("\tinterfaces: in %s", (char *)getifname(ips.is_ifp[0]));
	PRINTF(",%s", (char *) getifname(ips.is_ifp[1]));
	PRINTF(" out %s", (char *) getifname(ips.is_ifp[2]));
	PRINTF(",%s\n", (char *) getifname(ips.is_ifp[3]));
