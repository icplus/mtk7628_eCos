...
#  endif
//Eddy extern struct callout_handle ipfr_slowtimer_ch;
# endif
#endif
...
#include <machine/param.h> /* 29/10/03 Roger,  */	
#endif

extern unsigned int ipfr_slowtimer_ch;
...
ipfr_t *table[];
{
	...
	for (f = table[idx]; f; f = f->ipfr_next)
		if (!bcmp((char *)&frag.ipfr_src, (char *)&f->ipfr_src,
			...
			if (off == f->ipfr_off) {
				if (!(ip->ip_off & IP_MF))
					f->ipfr_ttl = 10;/* 23/07/04 Roger,  1 -> 10*/	
				else
					f->ipfr_off = atoff;
			}
