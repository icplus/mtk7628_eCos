 ...
 *
 * $Id: ip_proxy.h,v 1.1 2010/01/28 11:09:57 dennis Exp $
 */
...
#define	APR_DELETE	1
#define APR_DONOTCHANGEPORT		2
#define APR_WILDDST				4

#define	APR_ERR(x)	(((x) & 0xffff) << 16)
#define	APR_EXIT(x)	(((x) >> 16) & 0xffff)
#define	APR_INC(x)	((x) & 0xffff)

// musterc+, 2006.06.01: Solve the PASV mode problem between CuteFTP & Serv-U
//#define	FTP_BUFSZ	160
#define	FTP_BUFSZ	320
...
typedef struct  ftpside {
	char	*ftps_rptr;
	char	*ftps_wptr;
	void	*ftps_ifp; /* 15/01/04 Roger,  */	
	u_32_t	ftps_seq[2];
	u_32_t	ftps_len;
	int	ftps_junk;
	int	ftps_cmds;
	int	ftps_cmd;
	int     reply_feat;	// musterc+, 2006.06.01
...
struct virtual_aproxy {
	char	apr_label[APR_LABELLEN];	// Proxy label # 
	aproxy_t * apr;
	u_char	apr_p;		// protocol 
	u_short apr_port;	//port
	struct virtual_aproxy *next;
};

extern struct virtual_aproxy *virtual_aproxy_head;
#endif
...
extern	int	appr_new __P((fr_info_t *, struct nat *, aproxy_t *));
