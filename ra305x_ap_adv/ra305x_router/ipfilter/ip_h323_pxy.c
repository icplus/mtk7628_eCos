...
nat_t *nat;
{
	...
	if (find_port(ipaddr, data, datlen, &off, &port) == 0) {
		...
		ipn->in_p = IPPROTO_TCP;
		ipn->in_inip = nat->nat_inip.s_addr;
		ipn->in_inmsk = 0xffffffff;
		ipn->in_dport = htons(port);
		ipn->in_pmin = htons(port);
		ipn->in_pmax = htons(port);
		ipn->in_pnext = htons(port);
		ipn->ref_fr = nat->nat_fstate.rule;
		/* 
		...
		bcopy((u_char*)&nat->nat_outip.s_addr, data + off, 4);
	}
	return 0;
...
change_ip_only:
	if (find_port(ipaddr, data, datlen, &off, &port) == 0) {
		bcopy((u_char*)&nat->nat_outip.s_addr, data + off, 4);
		bcopy((u_char*)&nat->nat_outport, data + off+4, 2);
	}
...
virtual_s:
	...
	datlen = fin->fin_dlen - (tcp->th_off << 2);
	if (find_port(ipaddr, data, datlen, &off, &port) == 0) {
		bcopy((u_char*)&nat->nat_inip.s_addr, data + off, 4);
	}
...
int ippr_h245_init()
{
	bzero((char *)&h245_fr, sizeof(h245_fr));
	h245_fr.fr_ref = 1;
	h245_fr.fr_flags = FR_INQUE|FR_PASS|FR_QUICK|FR_KEEPSTATE;
...
nat_t *nat;
{
	...
	//if (find_port(ipaddr, data, datlen, &off, &port) == 0) {
	while ((datlen > 6) && find_port(ipaddr, data, datlen, &off, &port) == 0) {
		fr_info_t fi;
		nat_t     *ipn;
...
/*		port = htons(port); */
		...
		ipn = nat_outlookup(&fi, IPN_UDP|FI_W_DPORT, IPPROTO_UDP,
		...
		if (ipn == NULL) {
			...
			ipn = nat_new(&fi, &newip, nat_default_rule, NULL,
...
no_create_nat:
		...
		{
			bcopy((u_char*)&nat->nat_outip, data + off, 4);
		}
		data += 6;
		datlen -= 6; 
	}
