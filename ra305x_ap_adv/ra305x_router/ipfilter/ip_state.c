...
#include <sys/types.h>
#include <sys/param.h>
//#include <sys/file.h> /* 29/10/03 Roger,  */	
...
//# include <sys/fcntl.h> /* 29/10/03 Roger,  */	
...
//# include <sys/systm.h> /* 29/10/03 Roger,  */	
#endif
...
#include "ip_compat.h" /* 29/10/03 Roger,  */
...
//Eddy #  include <sys/libkern.h>
//Eddy #  include <sys/systm.h>
# endif
#endif
...
#define	TCP_MSL	90			/*  45 seconds */
//u_long	fr_tcpidletimeout = FIVE_DAYS,
u_long	fr_tcpidletimeout = 2 * 2 * 3600,    /* 2 hours */
	fr_tcpclosewait = 2 * TCP_MSL,
	fr_tcplastack = 2 * TCP_MSL,
	//fr_tcptimeout = 2 * TCP_MSL,
	fr_tcptimeout = 2 * 75,
	//fr_tcpclosed = 120,
	fr_tcpclosed = 2,	/* 1 second */
	//fr_tcphalfclosed = 2 * 2 * 3600,    /* 2 hours */
	fr_tcphalfclosed = 2 * TCP_MSL,    /* 90 seconds */
	fr_udptimeout = 240,
	fr_udpacktimeout = 24,
	fr_icmptimeout = 120,
...
int dir, fsm;
{
	...
	case TCPS_SYN_SENT: /* 2 */
		if ((flags & ~(TH_ECN|TH_CWR)) == TH_SYN) {
			 ...
			 */
			state[dir] = TCPS_FIN_WAIT_1;
			/* 30/09/04 Roger,  speed up nat expired*/		
	...
	case TCPS_FIN_WAIT_1: /* 6 */
		if ((flags & TH_ACK) && ostate > TCPS_CLOSE_WAIT) {
			 ...
			 */
			state[dir] = TCPS_TIME_WAIT;
			/* 03/07/04 Roger ++,   we speed up to delete NAT entry*/	
			//newage = fr_tcptimeout;
			newage = fr_tcpclosed;
			/* 03/07/04 Roger --,  */	
		} else
	...
	case TCPS_TIME_WAIT: /* 10 */
		/* 02/07/04 Roger ++,  we speed up to delete NAT entry */	
