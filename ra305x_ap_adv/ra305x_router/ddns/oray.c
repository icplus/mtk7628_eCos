...
static unsigned char zbuf[512+256+256];//[2048];
static unsigned char zdata[512+256];//[1024];
...
static char oray_xml_template1[] =
(
	"<?xml version=\"1.0\" encoding=\"utf-8\"?>\r\n"//40
	"<soap:Envelope xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" "//69
	"xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" "//45
	"xmlns:soap=\"http://schemas.xmlsoap.org/soap/envelope/\">\r\n"//57
	"<soap:Header>\r\n"//15
	"<AuthHeader xmlns=\"http://tempuri.org/\">\r\n"//42
	"<Account xmlns=\"http://tempuri.org/\">"
);

static char oray_xml_template2[] =
(
	"</Account>\r\n"//49
	"<Password xmlns=\"http://tempuri.org/\">"//13
);

static char oray_xml_template3[] =
(
	"</Password>\r\n"//51
	"</AuthHeader>\r\n"//15
	"</soap:Header>\r\n"//16
...
static char oray_xml_template4[] =
(
	"</soap:Body>\r\n"//14
	"</soap:Envelope>\r\n"
);
...
static int getmiscinfo(const char *userid, const char*pwd, struct hostent *site)
{
	...
	char *proc_blk;
	int proc_blk_len;
	char *_hostname = CONFIG_ORAY_SERVER;
	char *OrayVersion;
...
//	unsigned char tempbuf3[50];
	...
	ORAYDBG("WEBSERVICE server = %s\n id=%x, ver=%x, key=%x, clientinfo=%x\n", 
		_hostname, oray_clientid, oray_clientver, oray_clientkey, clientinfo);

	oray_port = ORAY_PORT;
	site = gethostbyname(_hostname);
	if (site == NULL)
