...
{
    if (Q) {
	syslog(LOG_ERR, "timer used before pim_callout_init()");
	exit(1);
    }
...
timer_nextTimer()
{
    if (Q) {
	if (Q->time < 0) {
	    syslog(LOG_WARNING, "timer_nextTimer top of queue says %d", 
		Q->time);
	    return 0;
	}
	return Q->time;
    ...
    void  	*data;  	/* what to call the timeout function with */
{
    ...
    if (node == 0) {
	syslog(LOG_WARNING, "Malloc Failed in timer_settimer\n");
	return -1;
    }
    node->func = action; 
    node->data = data;
