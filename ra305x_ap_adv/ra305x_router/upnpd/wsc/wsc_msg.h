...
#define DEF_WSC_ENVELOPE_TIME_OUT	5
typedef int (*envCallBack)(char *msg, int msgLen);

/* for compile issue */
struct _WscEnvelope;

typedef int (*httpCallBack)(struct _WscEnvelope *pEnvelope, UPNP_CONTEXT *context);
...
typedef int (*msgHandleCallback)(char *pMsgBuf, int msgLen, int interfaceIndex);
