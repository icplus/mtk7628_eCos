...
{
	if ((s+e)=="") { alert(Alerts[0]+m+Alerts[58]); return false;}
if (any=='*')
{
	if(((s=="*")&&(e!="")&&(e!="*"))||
		((e=="*")&&(s!="")&&(s!="*")))
	{
		alert(Alerts[0]+m+Alerts[58]);
		return false;
	}
	else if((s+e)=="*"||(s+e)=="**")
		return true;
...
if (any=='*')
{
	...
	if (t=='') {  alert(Alerts[0]+m+Alerts[19]); return 0; }
	var sn=Number(s);
	var en=Number(e);
	...
	if (sv && ev )
	{
		if (sn>en) { alert(m+Alerts[20]); return 0; }
		else return 1;
	}

	if (isNaN(sn)) { alert(Alerts[0]+m+Alerts[21]); return 0; }
	if (isNaN(en)) { alert(Alerts[0]+m+Alerts[22]); return 0; }
	if ((sn <=0 || sn>65535) || (en <=0 || en>65535))
	{
		alert(m+Alerts[23]);
		return 0;
	}
	alert(Alerts[0]+m+Alerts[19]);
	return 0;
}
...
function dateStrC(sec)
{
	...
	var str=d.getFullYear()+date[0]+(1+d.getMonth())+date[1]+d.getDate()+date[2]+ week[d.getDay()];
	return str;
}
