...
function ipMskChk(mn,str)
{
	...
	{
		m=mn.value.split('.');
		if (m.length!=4) { alert(Alerts[0]+str+Alerts[1]) ; return 0; }
	}

	var t = /[^0-9]{1,}/;
	for (var i=0;i<4;i++)
	{
		if (t.test(m[i])||m[i]>255) { alert(Alerts[0]+str+Alerts[1]) ; return 0; }
	}
	...
	for (k=0;k<32;k++)
	{
		...
		{
			alert(Alerts[0]+str+Alerts[1]) ;
			//for(var i=0; i<4; i++) m[i].value=m[i].defaultValue;
			return 0 ;
		}
	}
	if (f==0) { alert(Alerts[0]+str+Alerts[1]) ; return 0; }
	return 1 ;	
}
