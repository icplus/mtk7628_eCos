...
#define SSURV_LINE_LEN		(4+65+20+23+9+7+7+3)		// Channel+SSID+Bssid+Security+Signal+WiressMode+ExtCh+NetworkType
#define SSURV_LINE_LEN_WPS	(4+65+20+23+9+7+7+3+4+5)	// Channel+SSID+Bssid+Security+Signal+WiressMode+ExtCh+NetworkType+WPS+PIN

static void parse_site_survey_res(http_req *req, const char *res, int res_len)
{
	char *sp, *op;
	char site_chnl[4] = {0};
	char site_ssid[65] = {0};
	char site_ssid_raw[65] = {0};
	char site_bssid[24] = {0};
	char site_sec[24] = {0};
	char site_signal[10] = {0};
	int len;
#if !defined(WSC_AP_SUPPORT) && !defined(WSC_STA_SUPPORT)
	const int line_len = SSURV_LINE_LEN;
#else
	const int line_len = SSURV_LINE_LEN_WPS;
	...
	while (*sp && ((len - (sp - op)) >= 0))
	{
		...
		memcpy(site_ssid, sp + 4, 65);
		memcpy(site_bssid, sp + 69, 20);
		memcpy(site_sec, sp + 89, 23);
		memcpy(site_signal, sp + 112, 9);
		...
		/* TODO:
		 *	Pack it and add_to_response
		 */
		extern char *hexstr(const char *str, int len, char *buf, int buf_len);

		if (!strncmp("0x", site_ssid, 2)) {
			hexstr(site_ssid, strlen(site_ssid), site_ssid_raw, sizeof(site_ssid_raw));
		} else {
			strncpy(site_ssid_raw, site_ssid, sizeof(site_ssid_raw));
		}
		snprintf(tmp, sizeof(tmp), 
			"{\"channel\":\"%s\", \"ssid\":\"%s\", \"ssid_raw\":\"%s\", \"bssid\":\"%s\", \"security\":\"%s\", \"signal\":\"%s\"}",
			trim_r(site_chnl), trim_r(site_ssid), trim_r(site_ssid_raw), trim_r(site_bssid), trim_r(site_sec), trim_r(site_signal));
		if (first) {
			WEB_printf(req, "%s", tmp);
			first = !first;
		} else {
			WEB_printf(req, ",%s", tmp);
		}
...
static int CGI_do_sitesurvey(http_req *req)
{
#ifdef CONFIG_WIRELESS
#ifndef SIOCIWFIRSTPRIV
#define SIOCIWFIRSTPRIV 0x8BE0
#endif /* SIOCIWFIRSTPRIV */
#ifndef RTPRIV_IOCTL_GSITESURVEY
#define RTPRIV_IOCTL_GSITESURVEY (SIOCIWFIRSTPRIV + 0x0D)
#endif /* RTPRIV_IOCTL_GSITESURVEY */
...
#endif /* RTPRIV_IOCTL_FLAG_UI */

#define APCLI_INTF "apcli0"

	extern int CmdIwpriv(int argc, char *argv[]);
	struct iwreq param;
	cyg_netdevtab_entry_t *t;
	struct eth_drv_sc *sc = NULL, *_sc;
	char *tmp = NULL;
	int ret = 0;
	const char *argv[] = {APCLI_INTF, "set", "SiteSurvey=1"};

	/* Trigger Site Survey */
	CmdIwpriv(sizeof(argv) / sizeof(argv[0]), argv);
	...
	for (t = &__NETDEVTAB__[0]; t != &__NETDEVTAB_END__; t++) {
		_sc = (struct eth_drv_sc *)t->device_instance;
		if (strcmp(_sc->dev_name, APCLI_INTF) == 0) {
			sc = _sc;
			break;
		}
	}
	tmp = malloc(14336);//14*1024
	if (tmp) {
		memset(&param, 0x0, sizeof(param));
		tmp[0] = '\0';
		param.u.data.pointer = &tmp[0];
		param.u.data.flags |= (RTPRIV_IOCTL_FLAG_NOSPACE | RTPRIV_IOCTL_FLAG_NODUMPMSG);

#ifdef CONFIG_AP_SUPPORT
		ret = rt28xx_ap_ioctl(sc, RTPRIV_IOCTL_GSITESURVEY, (caddr_t)&param);
#endif /* CONFIG_AP_SUPPORT */
#ifdef CONFIG_STA_SUPPORT
		ret = rt28xx_sta_ioctl(sc, RTPRIV_IOCTL_GSITESURVEY, (caddr_t)&param);
#endif /* CONFIG_STA_SUPPORT */
		if (param.u.data.length != 0) {
			/* Try to parse and pack the result */
			tmp[param.u.data.length] = '\0';
			add_headers(req, HTTP_REQUEST_OK, "Ok", (char*) 0, "text/html; charset=", -1, -1, 1);
			send_response(req);
			parse_site_survey_res(req, tmp, param.u.data.length);
		} else {
			/* failed */
		}
		free(tmp);
...
int CGI_do_cmd(http_req *req)
{
	...
	if (!strcmp("WIRELESS_SITESURVEY", cmd)) {
		rc = CGI_do_sitesurvey(req);
	}
	...
	for (i = 0; i < 50; i++)
	{
		...
		*val=0;
		...
		if (id!=0)
		{
			...
			if (!forceToReboot) {
				/* Rebooting sovles these problems, ⊙﹏⊙ */
				if (!strcmp(CFG_id2str(id), "WLN_BssidNum"))
				{
					;//	forceToReboot = 1;
				}

				if (!strcmp(CFG_id2str(id), "WLN_ApCliAutoConnect")) {
					/* Use atoi here, incase somebody input an invalid string */
					apCliAutoConnect = atoi(val);
					if (apCliAutoConnect != 0) {
						/* There are memory-leaks in the net_cluster pool when trying to switch AutoConnect on,
						 * forcing reboot to avoid this for now. ⊙﹏⊙‖∣
						 */
						forceToReboot = 1;
					}
				} else if (!strcmp(CFG_id2str(id), "WLN_ApCliSsid")) {
					extern int opmode;

					switch (opmode) {
						default: /* Any other unexpected mode */
							diag_printf("[ WARN ] Unexpected router mode: %d\n", opmode);
						case 1: /* Gateway mode */
						case 3: /* Repeater mode */
							/* Though we can also manually call CmdIwpriv to change the ApClient ssid, 
							 * but the network segment of dhcp server may changed, so just simply call reboot.
							 */
							forceToReboot = 1;
							break;
						case 2: /* AP-Client mode */
							/* Just configuration reload is enough */
							break;
					}
				}
			}
			
			diag_printf("[ %s:%u ] [ DEBUG ] set %s(%u) = %s\n", __func__, __LINE__, 
						CFG_id2str(id), id, val);
...
ReadyToOut:	
			...
			strcmp(cmd, "WIRELESS_SIMPLE_CONFIG_DEVICE") && strcmp(cmd, "OPMODE"))
	{
...
#ifdef CONFIG_WIRELESS
		int __prev_ApCliAutoConnect = 0;

		if (CFG_get(CFG_WLN_ApCliAutoConnect, &__prev_ApCliAutoConnect) != -1
				&& __prev_ApCliAutoConnect == 1) {
			/* If ApCliAutoConnect is enabled, disabled it temporarily */
			diag_printf("[ DEBUG ] [ %s:%u ] Disable ApCliAutoConnect...\n", __func__, __LINE__);
			CFG_set_str(CFG_WLN_ApCliAutoConnect, "0");
		}
#endif /* CONFIG_WIRELESS */
		...
		CmdIwpriv(3,string);
		cyg_thread_delay(10);
		diag_printf("[ %s:%u ] DisConnectAllSta...\n", __func__, __LINE__);
		}
...
#endif /* CONFIG_BRIDGE */
#ifdef CONFIG_WIRELESS
		if (__prev_ApCliAutoConnect == 1) {
			/* Restore ApCliAutoConnect again. */
			diag_printf("[ DEBUG ] [ %s:%u ] Enable ApCliAutoConnect...\n", __func__, __LINE__);
			CFG_set_str(CFG_WLN_ApCliAutoConnect, "1");
		}
#endif /* CONFIG_WIRELESS */
...
#ifdef CONFIG_WIRELESS
		/* This command works only after wifi driver is up */
		if (apCliAutoConnect != -1 || __prev_ApCliAutoConnect) {
			extern int CmdIwpriv(int argc, char *argv[]);
			const char * args[] = {
				"apcli0", "set", "ApCliAutoConnect=0"
			};
			char tmp[64];

			//CmdIwpriv(sizeof(args) / sizeof(args[0]), args);
			if (apCliAutoConnect == -1)
				apCliAutoConnect = __prev_ApCliAutoConnect;
			snprintf(tmp, sizeof(tmp), "ApCliAutoConnect=%d", apCliAutoConnect);
			args[2] = &tmp;
			CmdIwpriv(sizeof(args) / sizeof(args[0]), args);
		}
#endif /* CONFIG_WIRELESS */
