...
function verifyMAC(ma,s,sp){
	...
	for (var i=0;i<6;i++)
	{
		if (!t.test(m[i])) { alert("無效的"+s+"!");  return false; }
    }
    if ((m[0]&0x01)||(m[0].toUpperCase()=="FF")) {	alert(s+"必須是單一的位址!"); return false;}
    return true
}
...
function ipMskChk(mn,str)
{
	...
	{
		m=mn.value.split('.');
		if (m.length!=4) { alert("無效的"+str+"!") ; return 0; }
	}

	var t = /[^0-9]{1,}/;
	for (var i=0;i<4;i++)
	{
		if (t.test(m[i])||m[i]>255) { alert("無效的"+str+"!") ; return 0; }
	}
...
{
    if (kk=='undefined') { alert("沒有定義"); return -1; }
