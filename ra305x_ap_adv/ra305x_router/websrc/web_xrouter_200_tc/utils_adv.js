...
{
	if ((s+e)=="") { alert("無效的"+m+"位址範圍!"); return false;}
if (any=='*')
{
	if(((s=="*")&&(e!="")&&(e!="*"))||
		((e=="*")&&(s!="")&&(s!="*")))
...
if (any=='*')
{
	...
	if (t=='') {  alert("無效的"+m+"連接埠範圍"); return 0; }
	var sn=Number(s);
	var en=Number(e);
	...
	if (sv && ev )
	{
		if (sn>en) { alert(m+"開始連接埠應小於或等於結束連接埠。"); return 0; }
		else return 1;
	}

	if (isNaN(sn)) { alert("無效的"+m+"開始連接埠"); return 0; }
	if (isNaN(en)) { alert("無效的"+m+"結束連接埠"); return 0; }
	if ((sn <=0 || sn>65535) || (en <=0 || en>65535))
	{
		alert(m+"連接埠值必須介於1到65535之間!");
		return 0;
	}
	alert("無效的"+m+"連接埠範圍");
	return 0;
...
function dateStrC(sec)
{
	...
	var str=d.getFullYear()+' 年 '+(1+d.getMonth())+' 月 '+d.getDate()+' 日 '+ week[d.getDay()];
	return str;
}
...
function chken(s, msg)
{
    ...
    for (i=0; i < s.length;i++)
    {
        ...
        if ((val<47) || ((val>57)&&(val<65)) || ((val>90)&&(val<97)) || (val>122))
        {
        	...
        	{
			alert(msg+" 請勿輸入字元, only accept [A~Z][a~z][0~9][-][_]");
			return false;        	
		}
        }
