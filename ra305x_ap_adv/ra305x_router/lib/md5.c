...
*/
       if (inputLen >= partLen) { 
       MD5_memcpy 
       ((unsigned char *)&context->buffer[index], (unsigned char *)input, partLen); MD5Transform
			 ...
			 int key_len)
{
  ...
  MD5_memset((unsigned char *)k_ipad, '\0', sizeof k_ipad);
  MD5_memset((unsigned char *)k_opad, '\0', sizeof k_opad);
  MD5_memcpy( k_ipad, (unsigned char *)key, key_len);
  MD5_memcpy( k_opad, (unsigned char *)key, key_len);
  ...
  MD5_memset((unsigned char *)&k_ipad, 0, sizeof(k_ipad));
  MD5_memset((unsigned char *)&k_opad, 0, sizeof(k_opad));
  MD5_memset((unsigned char *)&tk, 0, sizeof(tk));
			    ...
			    int key_len)
{
  ...
  MD5_memset((unsigned char *)&hmac, 0, sizeof(hmac));
}
...
{
  unsigned lupe;
  MD5_memset((unsigned char *)hmac, 0, sizeof(HMAC_MD5_CTX));
  for (lupe = 0; lupe < 4; lupe++) {
