...
static void do_sched_event(char *fun, char *file, int line, int lock);
...
diag_printf("\n\n");
	cyg_scheduler_lock();
	if(splx_thread)
		diag_printf("\nowner:%s\n",cyg_thread_get_name(splx_thread));
	//cyg_threadqueue queue_thread=splx_mutex.queue;
	cyg_thread *owner_thread=splx_mutex.owner;
	if(!cyg_mutex_threadqueue_empty(splx_mutex))
	{
		char* block_thread=cyg_mutex_threadqueue_highpri_name(splx_mutex);
		if(block_thread)
			diag_printf("block thread:%s\n",(block_thread));
	}
	if(owner_thread)
		diag_printf("mutex owner:%s\n",owner_thread->name);	
	cyg_scheduler_unlock();
...
#ifdef CYGIMPL_TRACE_SPLX
cyg_uint32
cyg_splinternal_debug(SPLXARGS)
#else
cyg_uint32
...
cyg_tsleep(void *chan, int pri, char *wmesg, int timo)
{
    ...
    if (timo) {
    	...
    	if((cyg_interrupt_bISR() || cyg_interrupt_bDSR()))				
		{	
