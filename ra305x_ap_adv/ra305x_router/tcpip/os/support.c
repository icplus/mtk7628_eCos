...
static void _show_hwaddr(const char *name, pr_fun *pr)
{
	struct ifreq ifr = {0};
	int s = socket(AF_INET, SOCK_DGRAM, 0);

	if (!(s < 0)) {
		strncpy(&ifr.ifr_name[0], name, sizeof(ifr.ifr_name));
		if (!(ioctl(s, SIOCGIFHWADDR, &ifr) < 0)) {
			(*pr)("Hwaddr %02x:%02x:%02x:%02x:%02x:%02x\n", 
