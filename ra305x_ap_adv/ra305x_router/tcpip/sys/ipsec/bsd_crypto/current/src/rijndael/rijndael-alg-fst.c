//==========================================================================
//
//      include/rijndael/rijndael-alg-fst.c
//
//==========================================================================
//####BSDCOPYRIGHTBEGIN####
//
...
#define	tk	xtk.x8
	...
	while (r < ROUNDS + 1) { /* while not enough round key material calculated */
		...
		tk[0][0] ^= cyg_S[tk[KC-1][1]];
		tk[0][1] ^= cyg_S[tk[KC-1][2]];
		tk[0][2] ^= cyg_S[tk[KC-1][3]];
		tk[0][3] ^= cyg_S[tk[KC-1][0]];
		...
		if (KC != 8) {
			...
			tk[KC/2][0] ^= cyg_S[tk[KC/2 - 1][0]];
			tk[KC/2][1] ^= cyg_S[tk[KC/2 - 1][1]];
			tk[KC/2][2] ^= cyg_S[tk[KC/2 - 1][2]];
			tk[KC/2][3] ^= cyg_S[tk[KC/2 - 1][3]];
			for (j = KC/2 + 1; j < KC; j++) {
				*((word32*)tk[j]) ^= *((word32*)tk[j-1]);
			}
		}
