...
#ifdef CONFIG_QOS
	sprintf(if_name,"%s%d",ifp->if_xname,ifp->if_unit);
	if (strcmp(if_name, CONFIG_DEFAULT_WAN_NAME) != 0)
	{
			struct schedq *sq;

			sq = (struct schedq *)malloc(sizeof(struct schedq), M_TEMP, M_NOWAIT);			
			if (sq == NULL) {
				diag_printf("%s Can't alloc the %s memory for schedq\n", __FUNCTION__, sizeof(struct schedq));
			} else {
				memset(sq, 0, sizeof(struct schedq));
				sq->enqueue = tfq_enqueue;
				sq->dequeue = tfq_dequeue;
				sq->request = tfq_requst;
				sq->ifq=  &ifp->if_snd;
				tfq_schedQ_register(ifp, sq);
			}
	}
#endif
...
eth_drv_stop(struct eth_drv_sc *sc)
{
  ...
  /*
  *the new wifi driver need alarm thread to do mcu ack of MCU CMD, alarm thread need spl mutex,
  *here we must release spl mutex, if not,the wifi driver will hang.
  */
	...
	/*
	*after wifi driver function is done,we must recover the spl metux to normal state.
	*we need to check which bit of 'spl_state' is set, and we set it again
	*/
	if (strcmp(sc->dev_name,"ra0") == 0)
	{
		if (s &0x1 )
...
// DEBUG
    ...
    case SIOCSIFFLAGS:
        if ((ifp->if_flags & IFF_UP) == 0 &&
            ...
            if ((ifp->if_flags & IFF_UP) != 0 &&
                ...
                eth_drv_start(sc);
#else
		/*  Call device ioctl to handle hardware flags  */
		if ((sc->funs->control)(sc, SIOCSIFFLAGS, data, 0 ) ) {
			error = EINVAL;
		}
#endif
            }
    ...
    default:
        error=(sc->funs->control)(sc, cmd, ifr, sizeof(struct ifreq));        
        break;
    }
