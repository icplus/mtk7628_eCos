...
static int
valid_hostname(const char *_hostname)
{
    const char * label;
    const char * label_end;

/*dliu,031031 : modified for members.3322.org */
#if 0
    if (!_hostname) {
        return false;
    }
...
static int 
build_qname(char *ptr, const char *_hostname)
{
    const char *label = _hostname;
    char *end_label;
    int total_len = 0;
    int len;
...
static int 
build_query(char * msg, const char * _hostname, short rr_type)
{
...
dot_hostname(const char *_hostname)
{
    struct hostent *hent = NULL;
    struct in_addr addr;

    if (inet_aton(_hostname, &addr)) {
        hent = alloc_hent();
        if (hent) {
            memcpy(hent->h_addr_list[0], &addr, sizeof(struct in_addr));
            hent->h_addrtype = AF_INET;
            hent->h_length = sizeof(struct in_addr);
            hent->h_name = alloc_string(strlen(_hostname)+1);
            if (!hent->h_name) {
                free_hent(hent);
                return NULL;
            }
            strcpy(hent->h_name, _hostname);
        }
    }
    return hent;
}
...
gethostbyaddr(const char *addr, int len, int type)
{
    ...
    sprintf(_hostname, "%d.%d.%d.%d.IN-ADDR.ARPA.",
    ...
    len = build_query(msg, _hostname, DNS_TYPE_PTR);
    if (len < 0) {
    ...
    len = send_recv(msg, len, sizeof(msg),AF_INET);
    if (len < 0) {
        //cyg_drv_mutex_unlock(&dns_mutex);
        CYG_REPORT_RETVAL( NULL );
        return NULL;
...
struct hostent *
gethostbyname(const char * _hostname)
{
    unsigned char msg[MAXDNSMSGSIZE];
    char name[256];
    struct hostent *hent;
    ...
    CYG_REPORT_FUNCARG1( "hostname=%08x", _hostname );
    ...
    CYG_CHECK_DATA_PTR( _hostname, "hostname is not a valid pointer!" );

    /* See if there is an answer to an old query. If so free the memory
       it uses */
    free_stored_hent();
  
    if (!valid_hostname(_hostname)) {
        /* It could be a dot address */
        hent = dot_hostname(_hostname);
        store_hent(hent);
        CYG_REPORT_RETVAL( hent );
        return hent;
    }
    ...
    memset(msg, 0, sizeof(msg));
    len = build_query(msg, _hostname, DNS_TYPE_A);
    ...
    if (domainname) {
        if ((strlen(_hostname) + strlen(domainname)) > 254) {
            h_errno = NO_RECOVERY;
            //cyg_drv_mutex_unlock(&dns_mutex);
            CYG_REPORT_RETVAL( NULL );
            return NULL;
