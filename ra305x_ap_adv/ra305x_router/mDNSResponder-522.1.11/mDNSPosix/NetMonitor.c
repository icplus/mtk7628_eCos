...
mDNSlocal void RecordHostInfo(HostEntry *entry, const ResourceRecord *const pktrr)
{
    if (!entry->host_name.c[0])
    {
        if (pktrr->rrtype == kDNSType_A || pktrr->rrtype == kDNSType_AAAA)
        {
            ...
            entry->NumQueries = 0;
            AssignDomainName(&entry->host_name, pktrr->name);
        }
        ...
        if (pktrr->rrtype == kDNSType_PTR)
            ...
            {
                entry->NumQueries = 0;
                AssignDomainName(&entry->host_name, &pktrr->rdata->u.name);
            }
    }
...
mDNSlocal void AnalyseHost(mDNS *const m, HostEntry *entry, const mDNSInterfaceID InterfaceID)
{
    ...
    {
        SendUnicastQuery(m, entry, &entry->host_name, kDNSType_HINFO, InterfaceID);
...
mDNSlocal void ShowSortedHostList(HostList *list, int max)
{
    ...
    for (e = &list->hosts[0]; e < end; e++)
    {
        ...
        if (e->host_name.c[0] || e->HIHardware.c[0] || e->HISoftware.c[0])
            mprintf("%##-45s %#-14s %#s\n", e->host_name.c, e->HIHardware.c, e->HISoftware.c);
    }
}
