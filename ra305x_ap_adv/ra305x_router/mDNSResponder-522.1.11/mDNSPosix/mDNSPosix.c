                                       ...
                                       mDNSIPPort dstPort, mDNSBool useBackgroundTrafficClass)
{
	...
	sin6->sin6_scope_id	=	thisIntf->index; //scope id is needed
    ...
    if (sendingsocket >= 0)
        err = sendto(sendingsocket, msg, (char*)end - (char*)msg, 0, (struct sockaddr *)&to, addr_size);
...
mDNSexport mStatus mDNSPlatformTCPConnect(TCPSocket *sock, const mDNSAddr *dst, mDNSOpaque16 dstport, domainname *host_name, mDNSInterfaceID InterfaceID,
                                          TCPConnectionCallback callback, void *context)
{
    (void)sock;         // Unused
    (void)dst;          // Unused
    (void)dstport;      // Unused
    (void)host_name;     // Unused
...
/*
*DNS_group_get() help to get eCos  DNS server from a system array
*/
extern in_addr_t DNS_group_get(int index);
...
for (i=0;i<MAXDNSSERVER;i++)
        {
            mDNSAddr DNSAddr;
            DNSAddr.ip.v4.NotAnInteger =DNS_group_get(i);
...
mDNSlocal int SetupSocket(struct sockaddr *intfAddr, mDNSIPPort port, int interfaceIndex, int *sktPtr)
{
    ...
    if (intfAddr->sa_family == AF_INET)
    {
        ...
        if (err == 0)
        {
            ...
            if (err < 0) { err = errno; perror("bind <AF_INET>"); fflush(stderr); }
        }
    ...
    else if (intfAddr->sa_family == AF_INET6)
    {
	    ...
	    bindAddr6.sin6_len = sizeof(bindAddr6);
            ...
            if (err < 0) { err = errno; perror("bind<AF_INET6>"); fflush(stderr); }
        }
    }     // endif (intfAddr->sa_family == AF_INET6)
#endif

    // Set the socket to non-blocking.
  #if 0 //eCos doesn't support much fcntl(),
	diag_printf("%s:%d	 err=%d\n",__FUNCTION__,__LINE__,err);
    if (err == 0)
    {
...
mDNSexport mStatus mDNSPlatformInit(mDNS *const m)
{
    ...
    if (err == mStatus_NoError)
    {
        ...
        if (err != mStatus_NoError)
        {
            fprintf(stderr, "mDNS WARNING: Unable to detect interface changes (%d).\n", err);
            err = mStatus_NoError;
        }
    }
...
mDNSexport void mDNSPosixGetFDSet(mDNS *m, int *nfds, fd_set *readfds, struct timeval *timeout)
{
    ...
    event_tick  = ( nextevent  - mDNS_Execute(m) );
    if (event_tick < 1) event_tick = 1;
    interval.tv_sec  = event_tick >> 10;                     // The high 22 bits are seconds
    interval.tv_usec = ((event_tick & 0x3FF) * 15625) / 16;  // The low 10 bits are 1024ths
