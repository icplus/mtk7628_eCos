...
static char host_name[MAX_ESCAPED_DOMAIN_NAME], hardware[256], software[256];
...
mDNSlocal void NameCallback(mDNS *const m, DNSQuestion *question, const ResourceRecord *const answer, QC_result AddRecord)
{
    ...
    {
        ConvertDomainNameToCString(&answer->rdata->u.name, host_name);
        StopNow = 1;
...
mDNSexport int main(int argc, char **argv)
{
    ...
    while (this_arg < argc)
    {
        ...
        host_name[0] = hardware[0] = software[0] = 0;
        ...
        else if (inet_pton(AF_INET6, arg, &s6) == 1)
        {
            ...
            if (strlen(arg) >= sizeof(host_name)) {
                fprintf(stderr, "hostname must be < %d characters\n", (int)sizeof(host_name));
                goto usage;
            }
            strcpy(host_name, arg);
        }

        // Now we have the host name; get its A, AAAA, and HINFO
        if (host_name[0]) DoQuery(&q, host_name, kDNSQType_ANY, &target, InfoCallback);
        if (StopNow == 2) break;
