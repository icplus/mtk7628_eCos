...
{
    union { struct sockaddr s; struct sockaddr_in a4; 
#if HAVE_IPV6
...
void  void ReadDDNSSettingsFromConfFile(mDNS *const m,  char *const filename, domainname *const host_name, 
	domainname *const domain, mDNSBool *DomainDiscoveryDisabled)
{
    ...
    if (f)
    {
        ...
        if (host_name && GetConfigOption(buf, "hostname", f) && !MakeDomainNameFromDNSNameString(host_name, buf)) goto badf;
...
mDNSexport void mDNSPlatformWriteLogMsg(const char *ident, const char *buffer, mDNSLogLevel_t loglevel)
{
    ...
    else                // else, in production mode, we write to syslog
    {
        ...
        default:
            diag_printf("Unknown loglevel %d, assuming LOG_ERR\n", loglevel);
            fflush(stderr);
        }
